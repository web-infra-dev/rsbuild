...
class PluginTypeError extends Error {
  constructor(plugin: unknown) {
    super(
      `Expect Rsbuild plugin instance to be an object, but got ${typeof plugin}.`,
    );
  }
}

class PluginMissingNameError extends Error {
  constructor() {
    super(`Rsbuild plugin should have a "name" field for identification.`);
  }
}

class WebpackPluginLikeError extends Error {
  constructor(plugin: object) {
    const name = plugin.constructor?.name ?? 'SomeWebpackPlugin';
    const message =
      `${color.yellow(name)} looks like a webpack or Rspack plugin, ` +
    ...
    super(message + color.green(example));
  }
}

class PluginSetupTypeError extends Error {
  constructor(plugin: unknown) {
    super(
...
class PluginNameDuplicatedError extends Error {
  constructor(plugin: RsbuildPlugin) {
    super(`Rsbuild plugin "${plugin.name}" registered multiple times.`);
  }
}

function assertPlugin(plugin: unknown): asserts plugin is RsbuildPlugin {
  if (!plugin || typeof plugin !== 'object') {
    throw new PluginTypeError(plugin);
  }
  const { name, setup, apply } = plugin as any;
  if (typeof name !== 'string') {
    logger.warn(new PluginMissingNameError());
  }
  if (typeof setup !== 'function') {
    if (typeof apply === 'function') {
      throw new WebpackPluginLikeError(plugin);
    } else {
      throw new PluginSetupTypeError(plugin);
    }
  }
}
  ...
  const addPlugins = (
    ...
    for (const plugin of newPlugins) {
      ...
      if (plugins.some((item) => item.name === plugin.name)) {
        logger.warn(new PluginNameDuplicatedError(plugin));
      }
      if (before) {
