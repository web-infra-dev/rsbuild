...
Rsbuild 配备了一个内置的开发服务器，当你执行 `rsbuild dev` 时，Rsbuild DevServer 将启动，并提供页面预览、路由、模块热更新等功能。

如果你希望将 Rsbuild DevServer 集成到已有 Server 中，可以通过该方法获取 Rsbuild DevServer 的实例方法进行按需调用。
...
export async function startDevServer() {
  ...
  return {
    close: async () => {
      // 在自定义 server 关闭时触发 Rsbuild DevServer 的关闭
...
import OnCloseDevServer from '@zh/shared/onCloseDevServer.md';
