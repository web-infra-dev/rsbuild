...
Rsbuild 提供了 SWC 插件和 esbuild 插件，让你能使用 SWC 或 esbuild 代替 babel-loader、ts-loader 和 terser 等库进行代码编译和压缩。详见：

- [SWC 插件文档](/plugins/list/plugin-swc.html)
- [esbuild 插件文档](/plugins/list/plugin-esbuild.html)
...
默认情况下，Rsbuild 使用 Babel 编译 TS 文件，开启 [tools.tsLoader](/config/options/tools.html#toolstsloader) 选项后，会使用 `ts-loader` 编译 TS 文件。
...
详见 [tools.tsLoader 文档](/config/options/tools.html#toolstsloader)。
...
这是一项实验性功能，在某些场景下可能无法正确工作，请查看 [experiments.lazyCompilation](/config/options/experiments.html#experiments-lazycompilation) 来了解具体用法。
...
详见 [output.disableSourceMap](/config/options/output.html#outputdisablesourcemap)。
