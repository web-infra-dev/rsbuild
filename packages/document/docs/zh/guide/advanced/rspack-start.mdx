...
如果你基于 Rsbuild 开发了一个前端框架，接入 rspack-provider 和 [接入 webpack-provider](/guide/start/quick-start.html#在前端框架中接入) 的方式类似，只需要把依赖从 `@rsbuild/webpack` 改为 `@rsbuild/rspack-provider` 即可。
