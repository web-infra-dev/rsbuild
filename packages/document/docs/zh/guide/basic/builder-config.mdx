...
- [Dev](/api/config-dev.html)：与本地开发有关的配置。
- [Html](/api/config-html.html)：与 HTML 有关的配置。
- [Tools](/api/config-tools.html)：与底层工具有关的配置。
- [Source](/api/config-source.html)：与源代码解析、编译方式相关的配置。
- [Output](/api/config-output.html)：与构建产物有关的配置。
- [Security](/api/config-security.html)：与安全有关的配置。
- [Performance](/api/config-performance.html)：与编译性能、产物性能有关的配置。
...
比如在 Modern.js 框架中，你可以直接在 [modern.config.ts](https://modernjs.dev/configure/app/usage.html) 文件里定义 Rsbuild 的 [source.alias](/api/config-source.html#sourcealias) 配置：
...
关于 Node API 的完整用法，请参考 [API - createRsbuild](/api/builder-core.html#createbuilder)。
...
打开生成的 `/dist/builder.config.js` 文件，即可查看 Rsbuild 配置的完整内容。
