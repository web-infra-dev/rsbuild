...
- [Server](/config/options/server)：与 Rsbuild 服务器有关的配置，在本地开发和预览时都会生效。
...
比如在 `rsbuild.config.ts` 中，你可以定义 Rsbuild 的 [source.alias](/config/options/source#sourcealias) 配置：
