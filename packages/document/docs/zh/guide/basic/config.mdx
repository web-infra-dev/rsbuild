...
- [Experiments](/config/options/experiments.html)：处于实验阶段的配置。
...
当你通过 Node API 调用 Rsbuild 时，你可以通过 Provider 的 `rsbuildConfig` 配置项来传入 Rsbuild 配置：
...
关于 Node API 的完整用法，请参考 [API - createRsbuild](/api/javascript-api/core.html#creatersbuild)。
