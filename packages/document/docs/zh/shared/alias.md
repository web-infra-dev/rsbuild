...
- 通过 [source.alias](http://rsbuild.dev/zh/config/options/source#sourcealias) 配置。
...
Rsbuild 提供了 [source.alias](http://rsbuild.dev/zh/config/options/source#sourcealias) 配置项，对应 Rspack 原生的 [resolve.alias](https://rspack.dev/zh/config/resolve#resolvealias) 配置，你可以通过对象或者函数的方式来配置这个选项。
...
你可以通过 [source.aliasStrategy](http://rsbuild.dev/zh/config/options/source#sourcealiasstrategy) 来调整这两个选项的优先级。
