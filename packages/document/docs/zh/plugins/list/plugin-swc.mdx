...
- `babel-plugin-import`：使用 [source.transformImport](/config/options/source.html#sourcetransformimport) 代替。
...
<PackageManagerTabs command="add @rsbuild/plugin-swc -D" />
...
import { builderPluginSwc } from '@rsbuild/plugin-swc';
...
:::tip
Rsbuild 提供了 [source.transformImport](/config/options/source.html#sourcetransformimport) 配置项，因此你不需要手动配置 `extensions.pluginImport`。
:::
