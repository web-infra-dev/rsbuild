...
默认情况下，页面实际生效的 rootFontSize 会根据当前页面的情况动态计算，如果希望指定页面实际生效的 rootFontSize，可关闭 `enableRuntime` 配置，并在[自定义 html 模版](/config/options/html#htmltemplate) 中自行注入 `document.documentElement.style.fontSize = '64px'`。
