...
因此，我们建议你使用 SWC 而不是 esbuild，用法请参考 [SWC 插件](/plugins/list/plugin-swc)。
...
Modern.js 框架默认集成了 Rsbuild 的 esbuild 插件，因此，你不需要手动安装和注册插件，只需要使用 [tools.esbuild](https://modernjs.dev/configure/app/tools/esbuild.html) 配置项即可：
...
如果你直接使用了 Rsbuild 的 Node API，那么需要手动安装和注册 esbuild 插件。
...
```js
import { builderPluginEsbuild } from '@rsbuild/plugin-esbuild';
...
将 `loader` 设置为 `false` 来关闭 esbuild 代码转译，此时 Rsbuild 会继续使用 Babel 来进行代码转译。
...
将 `minimize` 设置为 `false` 来关闭 esbuild 代码压缩，此时 Rsbuild 会继续使用 Terser 进行代码压缩。
...
虽然 esbuild 能给现有的 webpack 项目带来明显的构建性能提升，但这个工具在接入 Rsbuild 时还存在一定的局限性，需要大家在接入的时候格外注意。
...
使用 esbuild 进行代码转译时，诸如 `babel-plugin-import` 等原有 Babel 插件的语法编译功能在开启 esbuild 后无法使用。并且由于 Rsbuild 底层使用的是 esbuild 的 `Transform API`，因此不支持使用额外 esbuild 插件来进行自定义编译过程。
