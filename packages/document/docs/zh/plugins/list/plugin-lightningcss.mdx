...
使用 `lightningcss-loader` 替代 Rsbuild 内置的 `postcss-loader`，传递给 `lightningcss-loader` 的选项，请查阅 [lightningcss 仓库](https://github.com/parcel-bundler/lightningcss/blob/master/node/index.d.ts) 来了解具体用法。

- **类型：** `false | LightningCssLoaderOptions`
- **默认值：**
...
使用 Lightning CSS 来代替 Rspack 内置的 CSS 压缩工具进行 CSS 压缩，传递给 `lightningcss` 的选项，请查阅 [lightningcss 仓库](https://github.com/parcel-bundler/lightningcss/blob/master/node/index.d.ts) 来了解具体用法。

- **类型：** `false | LightningCssMinifyPluginOptions`
- **默认值：**
