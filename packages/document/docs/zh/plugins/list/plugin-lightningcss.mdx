...
需要注意的是，启用 Lightning CSS 作为 transform 工具后，基于 PostCSS 的配置和特性将无法使用，比如 [`tools.postcss`](../../config/tools/postcss)、[`tools.autoprefixer`](../../config/tools/autoprefixer) 和 Tailwind CSS。无法与 Tailwind CSS 联用的问题预计未来会在 TailwindCSS V4 中解决。
...
- **示例：** 通过 lightningcss 的自定义 vistor 将所有的长度单位转化为 rem。
