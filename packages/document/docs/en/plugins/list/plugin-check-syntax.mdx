...
If the corresponding source location is not shown in the log, try setting [output.disableMinimize](/config/output/disable-minimize) to true and rebuild again.
:::
