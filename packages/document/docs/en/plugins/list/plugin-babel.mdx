...
When using the Babel plugin, since the default Babel configuration will not have additional impact on the compilation results, Babel translation behavior will not be added without modifying the configuration. Babel configuration needs to be modified before it will take effect.
