...
By default, the actual rootFontSize of the page will be dynamically calculated based on the situation of the current page. If you want to specify the actual rootFontSize of the page, you can turn off the `enableRuntime` configuration and set it in [Customized html template](/config/options/html#htmltemplate) and inject `document.documentElement.style.fontSize = '64px'` by yourself.
