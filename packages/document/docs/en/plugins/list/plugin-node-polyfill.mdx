...
The Modern.js framework integrates the Rsbuild's Node Polyfill plugin by default, so you don't need to manually install and register the plugin, just set the [output.disableNodePolyfill](https://modernjs.dev/configure/app/output/disable-node-polyfill.html) config to `false`:
...
If you use the Rsbuild's Node API, you need to manually install and register the esbuild plugin.
...
```js
import { builderPluginNodePolyfill } from '@rsbuild/plugin-node-polyfill';
