...
<SourceCode href="https://github.com/web-infra-dev/rsbuild/tree/main/packages/plugin-source-build" />
...
When the Source Build plugin is registered, the Rsbuild will prioritize reading the file specified in the `source` field of the sub-project during the build process. Therefore, you need to configure the `source` field in the package.json file of the sub-project and point it to the source code file.
...
If the sub-project uses [exports](https://nodejs.org/api/packages#package-entry-points) field, you also need to add the `source` field in the `exports` field.
