...
When developing plugins for webpack-provider, please import `BuilderPluginAPI` from `@rsbuild/webpack-provider`.
...
When developing plugins for rspack-provider, please import `BuilderPluginAPI` from `@rsbuild/rspack-provider`.

```ts
import type { BuilderPlugin } from '@rsbuild/core';
import type { BuilderPluginAPI } from '@rsbuild/rspack-provider';
...
When developing plugins for both webpack-provider and rspack-provider, please import `DefaultBuilderPlugin` from `@rsbuild/shared`.
...
The content of `api.context` is exactly the same as `builder.context`, please refer to [builder.context](/api/javascript-api/builder-instance.html#builder-context).
