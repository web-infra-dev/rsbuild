...
:::tip 📢 Notice
The strategies in [Bundle Size Optimization](/guide/optimization/optimize-bundle.html) can also be used to improve build performance, so we won't repeat them here.
:::
...
If you have higher build performance requirements, you can easily switch to Rspack build mode, see [Using Rspack](/guide/advanced/rspack-start.html) for more information.
...
Rsbuild provides SWC plugin and esbuild plugin that allow you to use SWC or esbuild instead of babel-loader, ts-loader and terser for transformation and minification process. See:

- [SWC plugin document](/plugins/list/plugin-swc.html)
- [esbuild plugin document](/plugins/list/plugin-esbuild.html)
...
By default, Rsbuild uses Babel to compile TS files. After enabling the [tools.tsLoader](/config/options/tools.html#toolstsloader) option, `ts-loader` will be used to compile TS files.
...
See the [tools.tsLoader documentation](/config/options/tools.html#toolstsloader) for details.
...
This is an experimental feature and may not work correctly in some scenarios, see [experiments.lazyCompilation](/config/options/experiments.html#experiments-lazycompilation) for details.
...
This strategy is similar to ["Adjust Browserslist"](/guide/optimization/optimize-bundle.html#adjust-browserslist), the difference is that we can set different browserslist for development and production environment, thereby reducing the compilation overhead in the development environment.
...
See [output.disableSourceMap](/config/options/output.html#outputdisablesourcemap) for details.
