# Build Vue 3 App

Although the Modern.js framework is built on React, the underlying Rsbuild is not coupled with React. Therefore, you can use Rsbuild to build your Vue applications and leverage most of the capabilities provided by Rsbuild.

In this document, you will learn how to build a Vue 3 application using Rsbuild.

import Vue3Project from '@en/shared/vue3Project.md';

<Vue3Project />

import CliToolVue from '@en/shared/cliToolVue.md';

<CliToolVue />

## Using Vue Plugin

To compile Vue SFC (Single File Components) and JSX syntax, you need to register the Rsbuild [Vue plugin](/plugins/plugin-vue). The plugin will automatically add the necessary configuration for Vue builds and remove the built-in React-related configurations.

For example, register in `builder.config.ts`:

```ts title="builder.config.ts"
import { defineConfig } from '@rsbuild/cli';
import { builderPluginVue } from '@rsbuild/plugin-vue';

export default defineConfig({
  builderPlugins: [builderPluginVue()],
});
```

## Type Definitions

In a TypeScript project, you need to add type definitions for `*.vue` files so that TypeScript can recognize them correctly.

Create `env.d.ts` in the `src` directory and add the following content:

```ts title="src/env.d.ts"
declare module '*.vue' {
  import type { DefineComponent } from 'vue';

  // eslint-disable-next-line @typescript-eslint/ban-types
  const component: DefineComponent<{}, {}, any>;
  export default component;
}
```

import DeployApp from '@en/shared/deployApp.md';

<DeployApp />
