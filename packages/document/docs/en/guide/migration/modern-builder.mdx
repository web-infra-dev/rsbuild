...
It is worth noting that **some default values ​​and behaviors in Rsbuild have changed** compared to Builder:
...
- **TypeScript type check:** not enabled by default, you need to manually register the [Type Check Plugin](/plugins/list/plugin-type-check).
