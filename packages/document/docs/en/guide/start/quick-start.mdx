...
Except the `entry` option, the `createRsbuild` method also provides some other options, which you can learn more about in [API - createRsbuild](/api/javascript-api/builder-core.html#createbuilder).
...
To start local development, it is recommended to use the [builder.startDevServer](/api/javascript-api/builder-instance.html#builder-startdevserver) method, which will start a local Dev Server.
...
To deploy the App to production environment, it is recommended to use the [builder.build](/api/javascript-api/builder-instance.html#builder-build) method, which will build the production outputs.

```ts
await builder.build();
```

> For more introduction of Rsbuild instance methods, please read the [Rsbuild Instance](/api/javascript-api/builder-instance.html) chapter.
