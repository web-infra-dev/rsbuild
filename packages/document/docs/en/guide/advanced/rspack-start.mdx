...
:::Tip
When using Rspack for building, you can still configure Babel plugins through [Babel Plugin](/plugins/list/plugin-babel), but this will result in additional compilation overhead, which may slow down the Rspack build to some extent.
:::
