...
Integrating rspack-provider into a frontend framework is similar to integrating webpack-provider. You only need to change the dependency from `@rsbuild/webpack-provider` to `@rsbuild/rspack-provider`.
...
Rsbuild supports modifying Rspack config via [bundlerChain](/api/config-tools.html#toolsbundlerchain). configurations modified via bundlerChain will take effect on both webpack and Rspack builds.
...
Rspack mode does not support modifying CSS Modules configuration through `tools.cssLoader`, however, Rsbuild provides [output.cssModules](/api/config-output.html#outputcssmodules) configuration, it used to smooth Rspack mode and webpack mode difference.
...
| ----------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Browser Compatibility                     | Rsbuild will set different default values of Browserslist according to [build target](/guide/basic/build-target)                                                                                                                 | <ul><li>[browserslist](/guide/advanced/browserslist.html)</li></ul>                                                                                        |
| SourceMap generation                      | Rsbuild generates different SourceMaps according to the environment by default (false is used by default in Rspack CLI)                                                                                                          | <ul><li>[output.disableSourceMap](/api/config-output.html#outputdisablesourcemap)</li></ul>                                                                |
| SVG to React component                    | Rsbuild support to import SVG as component by default                                                                                                                                                                            | <ul><li>[Import SVG Assets](/guide/basic/svg-assets.html)</li><li>[output.disableSvgr](/api/config-output.html#outputdisablesvgr)</li></ul>                |
...
| Type Checker                              | Rsbuild runs TypeScript type checker by default                                                                                                                                                                                  | <ul><li>[Type checking](/guide/basic/typescript.html#type-checking)</li><li>[output.disableTsChecker](/api/config-output#outputdisabletschecker)</li></ul> |
...
| Integrate @rspack/html-plugin             | Rsbuild integrates `@rspack/html-plugin` by default to support more HTML template features (builtins.html is used by default in Rspack CLI)                                                                                      | <ul><li>[tools.htmlPlugin](/api/config-tools.html#toolshtmlplugin)</li></ul>                                                                               |
| DevServer                                 | Rsbuild implement DevServer based on [webpack-dev-middleware](https://github.com/webpack/webpack-dev-middleware) ([@rspack/dev-server](https://www.rspack.dev/guide/dev-server.html) is used in Rspack CLI)                      | <ul><li>[tools.devServer](/api/config-tools.html#toolsdevserver)</li></ul>                                                                                 |
...
By default, Rsbuild enables TypeScript type checking during build. In large projects, type checking can take much longer than the actual build time of Rspack, so this behavior can slow down the build speed to some extent during production environment builds.
