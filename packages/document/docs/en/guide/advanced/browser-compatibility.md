...
When you find that a third-party dependencies causes compatibility issues, you can add this dependency to Rsbuild's [source.include](/config/options/source#sourceinclude) config, Make Rsbuild do extra compilation for this dependency.
...
Please refer to [source.include](/config/options/source#sourceinclude) document for detailed usage guide.

### Downgrade the code out of the current project

When you import the code out of the current project, if the code has not been compiled, then you also need to configure [source.include](/config/options/source#sourceinclude) to it to compile.
...
In different usage scenarios, you may need different polyfill solutions. Rsbuild provides [output.polyfill](/config/options/output#outputpolyfill) config to switch between different polyfill modes.
...
When using this config, you need to ensure compatibility by yourself, such as manually import the required polyfill code through [source.preEntry](/config/options/source#sourcepreentry).
