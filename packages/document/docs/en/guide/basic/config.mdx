...
- [Server](/config/options/server)：Config related to Rsbuild Server, which will take effect during local development and preview.
...
For example, in `rsbuild.config.ts`, you can define the Rsbuild [source.alias](/config/options/source#sourcealias) configuration:
