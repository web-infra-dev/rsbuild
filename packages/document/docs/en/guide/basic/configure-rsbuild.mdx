...
export default defineConfig(({ env, command, mode }) => ({
  source: {
    alias: {
...
- `mode`: the value of the `env-mode` parameter for the current command being run.
  - When running `rsbuild build --env-mode test`, the value of `mode` is `test`.
