...
When `webpackProvider` is passed, the Rsbuild will use webpack as the bundler for building.
...
When `rspackProvider` is passed, the Rsbuild will use Rspack as the bundler for building.
