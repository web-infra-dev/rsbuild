...
By default, Rsbuild uses `@modern-js/utils/logger` to output logs. You can customize the log output object through the `logger` parameter.
...
```ts
const { builderConfig, bundlerConfigs } = await rsbuild.inspectConfig();
