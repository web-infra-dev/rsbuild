...
First you need to initialize the Rsbuild Provider and pass in the `rsbuildConfig` config object. Rsbuild provides a lot of configs that allow you to customize the build behavior. At this point, you don't need to know the specific content of the config, just pass in an empty object. You can find all available configs in [Config](/config/).
...
Except the `entry` option, the `createRsbuild` method also provides some other options, which you can learn more about in [API - createRsbuild](/api/javascript-api/core.html#creatersbuild).
