...
- 🌟 Support for customizing server based on the new JavaScript API.
- 🍭 Refactor of the SVGR plugin to support more usages.
- 📍 Support for custom compression options.
...
Rsbuild provides the [Lightning CSS plugin](/plugins/list/plugin-lightningcss) to use Lightning CSS on an opt-in basis, replacing the built-in PostCSS, autoprefixer, and SWC CSS minimizer in Rsbuild.
