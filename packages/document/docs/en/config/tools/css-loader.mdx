...
:::tip
When using Rspack as the bundler, this configuration is only supported when set [injectStyles](/config/output/inject-styles) is true.

To modify CSS Modules configuration, it is recommended to use the [output.cssModules](/config/output/css-modules) configuration.
:::
