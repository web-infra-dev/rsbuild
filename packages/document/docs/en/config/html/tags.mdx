...
type HtmlTag = {
  ...
  /**
   * Add this tag to the end / beginning of the original tags
   * @default true
   */
  append?: boolean;
  /**
...
- `head`: used to describe whether to add this tag to the page `<head>`
...
This configuration is used to modify the content of HTML files after Rsbuild completes building, and does not resolve or parse new modules. It cannot be used to import uncompiled source code files. Also cannot replace configurations such as [source.preEntry](/config/source/pre-entry).
