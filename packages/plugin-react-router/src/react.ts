...
export const applyBasicReactSupport = (
  ...
  api.modifyBundlerChain(
    async (chain, { CHAIN_ID, environment, isDev, target }) => {
      ...
      // const isSSR = Boolean(api.context.bundlerConfigs?.node);
      const isSSR = true;
      if (!usingHMR || !options.fastRefresh || isSSR) {
        return;
      }
