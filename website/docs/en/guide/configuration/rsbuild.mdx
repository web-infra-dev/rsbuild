---
description: 'Rsbuild provides a wide range of configuration options with sensible defaults for most use cases.'
---
...
You can find detailed descriptions of every option on the [Config overview](/config/) page.
...
:::tip
If Node.js shows the `[MODULE_TYPELESS_PACKAGE_JSON]` warning for `rsbuild.config.ts`, you can add `"type": "module"` to package.json or rename the file to `rsbuild.config.mts` to resolve the warning.
:::
