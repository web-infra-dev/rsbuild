...
When you use the CLI of Rsbuild, it automatically reads the configuration file in the root directory of the current project and resolves it in the following order:
...
For example, if you need to use the `rsbuild.prod.config.mjs` file when running `build`, add the following script to `package.json`:
