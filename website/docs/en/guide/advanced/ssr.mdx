  ...
  environments: {
    ...
    node: {
      ...
      output: {
        ...
        target: 'node',
        // Let Node.js load runtime dependencies from node_modules
        autoExternal: true,
      },
    },
  },
...
In SSR scenarios, the server bundle usually runs in Node.js and is deployed together with `node_modules`. For this setup, prefer [output.autoExternal](/config/output/auto-external) over manually listing packages in `output.externals`. Rsbuild will externalize dependencies declared in the root `package.json`, including subpath imports such as `react/jsx-runtime`, so runtime packages, native addons, and libraries that rely on runtime instrumentation can be loaded directly by Node.js. If you need a fully standalone server bundle, or if the deployment environment does not install the externalized dependencies, keep this option disabled or customize which dependency types are externalized.
