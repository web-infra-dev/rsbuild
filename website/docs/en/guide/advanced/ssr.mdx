...
This chapter introduces how to use Rsbuild to quickly create an SSR application.
...
Rsbuild does not have built-in SSR rendering capabilities, but you can quickly implement an SSR rendering function through Rsbuild's [custom server](/guide/basic/server#custom-server) and [environment API](/guide/advanced/environments#environment-api) capabilities:
...
Now, you can execute the `npm run dev` command to start the development server with SSR rendering function, and visit `http://localhost:3000/` to see that the SSR content has been rendered to the html template.
