...
Here's the professional English translation of the selected text:
...
By default, Rsbuild uses [SWC](/guide/configuration/swc) to compile and transform source code in the current project, as well as all TS and JSX modules in all directories. Other modules are not processed by SWC.
