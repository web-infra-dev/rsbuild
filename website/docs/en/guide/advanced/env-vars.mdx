...
By default, Rsbuild injects some environment variables into the code using [source.define](#using-define). These are replaced with specified values during the build:
...
We recommend using `--env-mode` to set the environment mode instead of modifying `process.env.NODE_ENV`.
