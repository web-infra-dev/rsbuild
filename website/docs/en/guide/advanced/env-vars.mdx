...
You can use `import.meta.env.BASE_URL` in the client code to access the server's [base path](/guide/basic/server#base-path), which is determined by the [server.base](/config/server/base) configuration.
...
In JS files, you can also use `import.meta.env.BASE_URL`:
