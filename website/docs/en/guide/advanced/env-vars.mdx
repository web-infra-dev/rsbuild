...
You can use `process.env.BASE_URL` in client code to access the server's [base path](/guide/basic/server#base-path). Rsbuild will automatically remove the trailing slash from base path to make string concatenation easier.
