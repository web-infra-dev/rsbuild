...
When you use Rsbuild [JavaScript API](/api/start/index), you can manually call the [loadEnv](/api/javascript-api/core#loadenv) method to read environment variables and inject them into the code through the [source.define](/config/source/define) configuration.
