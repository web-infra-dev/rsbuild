...
You can use `process.env.PUBLIC_BASE_PATH` in client code to access the server's [base path](/guide/basic/server#base-path). Rsbuild will automatically remove the trailing slash from base path to make string concatenation easier.
...
Then, the access path to the `favicon.ico` file in the public directory is `http://localhost:3000/foo/favicon.ico`. We can reference `./public/favicon.ico` file in the HTML template in the following way:

```html title="index.html"
<link rel="icon" href="<%= process.env.PUBLIC_BASE_PATH %>/favicon.ico" />
```
