---
description: 'This document explains how to use Rsbuild to build a React application.'
---
...
- Recommended: enable the Rust-based React Compiler through [@rsbuild/plugin-react](/plugins/list/plugin-react#reactcompiler). This uses Rspack's built-in SWC loader and provides a simpler setup and better build performance.
- Optional: use [React Compiler with @rsbuild/plugin-babel](/plugins/list/plugin-babel#use-react-compiler) if you are maintaining an existing Babel-based setup, using an older version of Rsbuild, or need Babel-plugin-based customization.

### How to use

If you are using Rsbuild 2.1.0 or later, you can enable the Rust-based React Compiler directly through `@rsbuild/plugin-react`:
...
This is the recommended approach because it provides a simpler setup and better build performance.

The `reactCompiler: true` option enables the Rust-based React Compiler through Rspack's built-in SWC loader. For details, see the [Rspack React Compiler guide](https://rspack.rs/guide/tech/react#using-builtinswc-loader).
...
This option enables the React Compiler through Rspack's built-in SWC loader. See [more details](/plugins/list/plugin-react#reactcompiler).
