...
Rsbuild is a high-performance build tool powered by Rspack. It provides thoughtfully designed default build configurations for an out-of-the-box development experience that fully leverages Rspack's performance advantages.
...
- The underlying bundler switched from webpack to Rspack, delivering 5-10x better build performance.
...
- It provides better extensibility. You can extend Rsbuild flexibly through [configuration](/config/index), [Plugin API](/plugins/dev/index), and [JavaScript API](/api/start/index).
...
- **Easy to Configure**: One of Rsbuild's goals is to provide out-of-the-box build capabilities for Rspack users, allowing developers to start web projects with zero configuration. Rsbuild also provides semantic build configuration to reduce the learning curve for Rspack.

- **Performance Oriented**: Rsbuild integrates high-performance Rust-based tools from the community, including [Rspack](https://rspack.rs), [SWC](https://swc.rs/) and [Lightning CSS](https://lightningcss.dev/), delivering top-tier build speed and development experience.
...
- **Stable Artifacts**: Rsbuild prioritizes build artifact stability. It ensures high consistency between artifacts in development and production builds, and automatically handles syntax downgrading and polyfill injection. Rsbuild also provides plugins for type checking and artifact syntax validation to prevent quality and compatibility issues in production code.
