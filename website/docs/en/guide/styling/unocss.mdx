---
description: 'UnoCSS is the instant atomic CSS engine, which is designed to be flexible and extensible.'
---
...
You can register the `unocss` PostCSS plugin through [postcss.config.mjs](https://github.com/webpack/postcss-loader#config) or [tools.postcss](/config/tools/postcss).
...
Rsbuild uses [Lightning CSS](/guide/styling/css-usage#lightning-css) to add vendor prefixes by default, so you only need to register the `UnoCSS` plugin.
