...
Rsbuild also provides several configurations that allow you to customize the processing rules for CSS files.

## Lightning CSS

:::tip
[Lightning CSS](https://lightningcss.dev) is a high-performance CSS parser, transformer and minifier written in Rust. It supports parsing and transforming many modern CSS features into syntax supported by target browsers, and provides better compression ratios.
:::
...
- Lightning CSS automatically adds vendor prefixes like `-webkit-`, `-moz-`, `-ms-`, etc., so you do not need to manually add prefixes or use the [autoprefixer](https://github.com/postcss/autoprefixer) plugin.
