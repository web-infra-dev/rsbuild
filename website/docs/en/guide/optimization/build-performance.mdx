...
:::tip
The methods in [Bundle Size Optimization](/guide/optimization/optimize-bundle) can also be used to improve build performance, so we won't repeat them here.
:::
...
The following are some general optimization methods, which can speed up the development build and production build, and some of them also optimize the bundle size.
