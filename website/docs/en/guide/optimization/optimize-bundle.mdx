...
In typical front-end projects, images often account for a large portion of total bundle size. Reducing image size can meaningfully lower the overall bundle size. Enable image compression by registering a plugin in Rsbuild:
...
A good chunk splitting strategy is important for improving application loading performance. It takes advantage of browser caching to reduce the number of requests and improve loading speed.
