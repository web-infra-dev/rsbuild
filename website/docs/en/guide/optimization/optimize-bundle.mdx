...
We recommend lightweight libraries in your project, such as replacing [moment](https://momentjs.com/) with [day.js](https://day.js.org/).
...
Rsbuild compiles code based on your project's browserslist config and injects polyfills. If your project doesn't need to support legacy browsers, adjust the browserslist to drop older targets and reduce compilation overhead for syntax transforms and polyfills.
...
A good chunk splitting strategy improves application loading performance. It uses browser caching to reduce the number of requests and speed up loading.
