...
Rsbuild uses the `split-by-experience` strategy by default, an optimization strategy based on practical experience. When the following npm packages are used in your project, they are automatically split into separate chunks:
...
Dynamic import is a feature introduced in ECMAScript 2020 that allows you to dynamically load JavaScript modules. Rspack, which powers Rsbuild, supports dynamic import by default, so you can use it directly in your code.
:::
