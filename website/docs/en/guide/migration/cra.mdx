---
description: 'This chapter introduces how to migrate a Create React App (CRA) or CRACO project to Rsbuild.'
---
...
For coding agents that support Skills, install the [migrate-to-rsbuild](https://github.com/rstackjs/agent-skills#migrate-to-rsbuild) skill to help with the migration process.
...
First, replace the npm dependencies of CRA with Rsbuild's dependencies.
...
Next, update the npm scripts in package.json to Rsbuild's CLI commands.
...
:::tip
Rsbuild doesn't include built-in testing frameworks, so it does not provide a command to replace `react-scripts test`. Use testing frameworks such as [Rstest](https://github.com/web-infra-dev/rstest), Jest or Vitest directly. Check the [Testing](/guide/advanced/testing) section for more details.
:::
...
In the HTML template, replace CRA's `%PUBLIC_URL%` variable with Rsbuild's [assetPrefix variable](/config/html/template-parameters) and use a forward slash for concatenation:
...
Configure Rsbuild's [output.distPath.root](/config/output/dist-path) option to change the directory to `build`, in line with CRA:
...
If your CRA project uses the "SVG to React Component" feature (i.e., [SVGR](https://react-svgr.com/)), also install the SVGR plugin for Rsbuild.
...
In CRA, if you import an asset that the build tool cannot recognize, CRA will by default output the file to the `build/static/media` directory, for example, the `model.gltf` file:
...
3. Configure [output.polyfill](/config/output/polyfill):
...
CRA uses ESLint to lint the code by default. Rsbuild does not include linting by default, but ESLint can be run through a separate npm script in `package.json`.

First, install [eslint](https://eslint.org/) and [eslint-config-react-app](https://npmjs.com/package/eslint-config-react-app). Use ESLint v8 to stay consistent with CRA:
...
Then run `npm run lint` to run ESLint.

### Run ESLint during the build

In addition to adding the `lint` command in `package.json`, add the [@rsbuild/plugin-eslint](https://github.com/rstackjs/rsbuild-plugin-eslint) plugin to maintain the same behavior as CRA.
...
After registration, ESLint runs automatically during development (`npm run dev`) and production builds (`npm run build`). The plugin displays ESLint warnings and errors in the console output.
...
export default {
  tools: {
    rspack: {
      ...
      plugins: process.env.NODE_ENV === 'development' ? [new MyWebpackPlugin()] : [],
    },
  },
};
```
