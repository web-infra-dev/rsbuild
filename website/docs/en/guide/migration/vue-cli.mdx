---
description: 'This chapter introduces how to migrate a Vue CLI project to Rsbuild.'
---
...
First, replace the npm dependencies of Vue CLI with Rsbuild's dependencies.
...
Next, update the npm scripts in the package.json file to Rsbuild's CLI commands.
...
:::tip
Rsbuild does not integrate ESLint, so it does not provide a command to replace `vue-cli-service lint`. Use ESLint's [CLI commands](https://eslint.org/docs/latest/use/command-line-interface) directly as an alternative.
:::
...
Vue CLI uses the `public/index.html` file as the default HTML template. In Rsbuild, specify the HTML template through [html.template](/config/html/template):
...
In the HTML template, replace Vue CLI's `BASE_URL` variable with Rsbuild's [assetPrefix variable](/config/html/template-parameters) and use a forward slash for concatenation:
...
This completes the basic migration from Vue CLI to Rsbuild. Run `npm run serve` to start the dev server.
