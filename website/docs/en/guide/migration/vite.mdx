...
Rsbuild does not support Vite's `?worker&url`, `?sharedworker`, `?sharedworker&inline`, or `?sharedworker&url` query suffixes. If you only use these queries to create a worker, migrate to the standard constructor syntax instead:
...
The constructor syntax also lets you pass standard `WorkerOptions`, such as `name`, `type`, and `credentials`. See [Web Workers](/guide/basic/web-workers) for more details.
