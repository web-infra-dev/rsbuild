---
description: 'Rsbuild supports importing static assets, including images, fonts, media, and other file types.'
---

# Static assets

Rsbuild supports importing static assets, including images, fonts, media, and other file types.
...
For a more detailed introduction to asset inlining, refer to the [Inline static assets](/guide/optimization/inline-assets) section.
...
For example, in an HTML template, the `./public/favicon.ico` file can be referenced as `/favicon.ico`. [BASE_URL](/guide/advanced/env-vars#processenvbase_url) is the base path of the server.
...
console.log(myFile); // "/static/model.[hash].gltf"
```
