...
For example, in an HTML template, the `./public/favicon.ico` file can be referenced as `/favicon.ico`, [PUBLIC_BASE_PATH](/guide/advanced/env-vars#processenvpublic_base_path) is the base path of the server.
