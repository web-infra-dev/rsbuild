...
Rsbuild includes a built-in development server designed to improve the development experience. When you run `rsbuild dev` or `rsbuild preview`, the server starts and provides features like page preview, routing, and hot module replacement.
...
You can modify the server's base path using [server.base](/config/server/base). To access files at `http://localhost:3000/foo/`, configure:
...
After starting the development server, you can access `/rsbuild-dev-server` to view all static assets generated during the current build.
