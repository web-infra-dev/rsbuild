...
Rsbuild comes with a built-in dev server designed to improve the development experience. When you run the `rsbuild dev` or `rsbuild preview` commands, the server starts and provides features such as page preview, routing, and hot module reloading.
...
Rsbuild server provides a set of default routing conventions and allows users to customize them through configurations.
...
When Rsbuild's default [server.htmlFallback](/config/server/html-fallback) configuration doesn't meet your needs, for example, if you want to access `main.html` when accessing `/`, you can configure it using [server.historyApiFallback](/config/server/history-api-fallback).
