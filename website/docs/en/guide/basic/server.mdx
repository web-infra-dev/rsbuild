...
If Rsbuild's default [server.htmlFallback](/config/server/html-fallback) configuration doesn't meet your needs—for example, you want to access `main.html` when visiting `/`—configure it using [server.historyApiFallback](/config/server/history-api-fallback):
