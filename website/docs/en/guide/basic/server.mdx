...
When migrating from other server frameworks (such as Express), the original middleware may not work directly in Rsbuild. For example, properties like `req.params`, `req.path`, `req.search`, and `req.query` provided by Express are not accessible in Rsbuild middleware.
