...
Rsbuild supports modifying the base path of the server through [server.base](/config/server/base). If you want to access these files through `http://localhost:3000/foo/`, you can configure the following:
...
Rsbuild server offers a set of default routing conventions, and allows users to customize it through configurations.
...
In Rsbuild, `dev` contains some configurations that only work in development mode, while the `server` config works for both dev and preview servers.
