...
| `--log-level <level>`      | Set the log level, can be `info`, `warn`, `error` or `silent`, see [logLevel](/config/log-level)                                  |
| `-m, --mode <mode>`        | Set the build mode (`development`, `production` or `none`), see [mode](/config/mode)                                              |
