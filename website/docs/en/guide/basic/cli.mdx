...
Rsbuild CLI includes several common flags that work with all commands:
...
The `rsbuild dev` command starts a local dev server and compiles the source code for development.
...
The `--open` option automatically opens a page when starting the dev server, which is equivalent to setting [server.open](/config/server/open) to `true`.
