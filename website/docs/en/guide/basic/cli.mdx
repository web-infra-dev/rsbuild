...
Options:
  -o, --open [url]      Open the page in the browser on startup
  --port <port>         Set the port number for the server
  --host <host>         Set the host the server listens on
```
...
Options:
  ...
  --port <port>         Set the port number for the Rsbuild server to listen on
  --host <host>         Set the host the Rsbuild server listens on
```
