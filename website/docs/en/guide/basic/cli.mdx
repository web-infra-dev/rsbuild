...
The `rsbuild dev` command starts a local development server and compiles source code for development.
...
You can also start the development server by running `rsbuild` directly, which is equivalent to running `rsbuild dev`.
...
The `--open` option automatically opens a page when starting the development server, equivalent to setting [server.open](/config/server/open) to `true`.
...
The `rsbuild preview` command previews the production build outputs locally. Note that you need to execute the `rsbuild build` command beforehand to generate the build outputs.
...
The `rsbuild inspect` command displays the Rsbuild config and Rspack config of the project.
