...
This shows the basic output file directory structure. By default, compiled files are output to the `dist` directory of the current project.
...
When you import static assets such as images, SVG, fonts, media, etc., in the code, they are output to the `dist/static` directory and automatically organized into subdirectories by file type:
