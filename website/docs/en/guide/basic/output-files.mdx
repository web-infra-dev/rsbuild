...
When importing static assets (images, SVG, fonts, media, etc.) in your code, they are written to the `dist/static` directory and automatically organized into subdirectories by file type:
...
Use the [output.distPath](/config/output/dist-path) configuration to write these static assets to a single directory. For example, to organize them in an `assets` directory:
...
When [output.target](/config/output/target) is set to `'node'`, Rsbuild generates output files for Node.js:
