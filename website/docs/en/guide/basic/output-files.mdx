...
- License files: contain open source licenses, which are output to the same level directory as the JS file, and add `.LICENSE.txt` suffix.
- Source Map files: contain the source code mappings, which are output to the same level directory as JS files and CSS files, and add a `.map` suffix.
