...
The default output directory structure is shown below. Output files are generated in the `dist` directory of the current project.
...
- HTML files: generated in the root of the dist directory by default.
- JS files: generated in the `static/js` directory by default.
- CSS files: generated in the `static/css` directory by default.

Additional files may be generated alongside JS and CSS files:

- License files: contain open source license information, generated in the same directory as the JS file with a `.LICENSE.txt` suffix.
- Source map files: contain source code mapping information, generated in the same directory as JS and CSS files with a `.map` suffix.
...
When importing static assets such as images, SVG, fonts, media, etc., in your code, they are generated in the `dist/static` directory and automatically organized into subdirectories by file type:
...
Use the [output.distPath](/config/output/dist-path) config to store these static assets in a single directory. For example, to organize them in an `assets` directory:
...
For example, to store Node.js files in the `server` directory:
