...
To write output files to disk—useful for inspecting build artifacts or configuring proxy rules for static assets—set [dev.writeToDisk](/config/dev/write-to-disk) to `true`:
...
Rsbuild provides several options to customize directories or filenames:
