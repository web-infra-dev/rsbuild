...
This chapter introduces the directory structure of output files and how to control the output directory of different types of files.
...
The following section shows the basic directory structure for output files. By default, the compiled files will be output in the `dist` directory of the current project.
...
- HTML files: default output to the root of the dist directory.
...
- License files: contain open source licenses, which are output to the same directory as the JS file, and have `.LICENSE.txt` suffix.
- Source map files: contain the source code mappings, which are output to the same directory as JS files and CSS files, and have a `.map` suffix.
...
- Modify source map files through [output.sourceMap](/config/output/source-map).
...
You can use the [output.distPath](/config/output/dist-path) config to uniformly output these static assets into a directory, for example, output to the `assets` directory:
