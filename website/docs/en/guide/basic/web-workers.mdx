...
When your application contains both page code and Web Workers code, it's important to note that Web Workers run in a separate thread, isolated from the main thread environment. The polyfill [entry mode](/guide/advanced/browser-compatibility#entry-mode) provided by Rsbuild cannot take effect in Web Workers. In this case, it is recommended to use the [usage mode](/guide/advanced/browser-compatibility#usage-mode) to inject the required polyfill code specifically for the Web Workers.
