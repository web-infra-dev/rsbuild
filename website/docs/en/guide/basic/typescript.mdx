...
`@rsbuild/core` includes some preset type definitions, including CSS files, CSS Modules, static assets, `import.meta` and other types.
...
Rsbuild includes the Type Check plugin, which runs TypeScript type checking in a separate process. The plugin internally integrates [ts-checker-rspack-plugin](https://github.com/rspack-contrib/ts-checker-rspack-plugin).
