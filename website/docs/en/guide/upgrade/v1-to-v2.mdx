...
- Upgrade `@rsbuild/core` to the 2.0 Beta release:
...
Rsbuild has upgraded its dependency on [http-proxy-middleware](https://github.com/chimurai/http-proxy-middleware) from v2 to v3 and some options have changed.

You can follow the examples below to migrate your config, or check the [http-proxy-middleware v3 migration guide](https://github.com/chimurai/http-proxy-middleware/blob/master/MIGRATION.md#v3-breaking-changes) for more details.
...
- Removed the deprecated [content-changed](/api/javascript-api/server-api#sockwrite) message type from `sockWrite`, use `static-changed` instead.
