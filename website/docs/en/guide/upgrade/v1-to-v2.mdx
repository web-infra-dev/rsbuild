---
description: 'This document lists all breaking changes from Rsbuild v1 to 2.0. Use it as a migration reference.'
---
...
This document lists all breaking changes from Rsbuild v1 to 2.0. Use it as a migration reference.

## Using Agent Skills

If you are using a coding agent that supports Skills, install the [rsbuild-v2-upgrade](https://github.com/rstackjs/agent-skills#rsbuild-v2-upgrade) skill to help with the upgrade process from v1 to v2.
...
Rsbuild v2 now depends on [@rspack/core](https://www.npmjs.com/package/@rspack/core) v2. If you use custom Rspack configurations or plugins, you may need to make corresponding adjustments.
...
### Default decorators version

The default value of [source.decorators.version](/config/source/decorators#decoratorsversion) has changed from `2022-03` to `2023-11`.
...
When [output.target](/config/output/target) is set to `node`, Rsbuild 2.0 defaults to ESM output via [output.module](/config/output/module) and keeps [output.minify](/config/output/minify) disabled. In Rsbuild v1, the default was CommonJS output with minification enabled.
...
### Migrate `performance.chunkSplit`
...
- `strategy: 'split-by-size'` → configure `minSize` / `maxSize` directly
...
### Proxy middleware upgraded

Rsbuild has upgraded its dependency on [http-proxy-middleware](https://github.com/chimurai/http-proxy-middleware) from v2 to v4 and some options have changed.

You can follow the examples below to migrate your config, or check the [http-proxy-middleware v3 breaking changes](https://github.com/chimurai/http-proxy-middleware/blob/master/MIGRATION_V3.md#v3-breaking-changes) for more details.

- The `context` option is replaced by `pathFilter`:
...
- Removed the deprecated [content-changed](/api/javascript-api/server-api#sockwrite) message type from `sockWrite`. Use `full-reload` instead.
- Renamed `rsbuild.onAfterStartProdServer` method to `rsbuild.onAfterStartPreviewServer`
- Renamed `rsbuild.onBeforeStartProdServer` method to `rsbuild.onBeforeStartPreviewServer`
...
- Renamed `api.onAfterStartProdServer` hook to `api.onAfterStartPreviewServer`
- Renamed `api.onBeforeStartProdServer` hook to `api.onBeforeStartPreviewServer`

## Dropping webpack support

Rsbuild 2.0 no longer supports using webpack as the bundler. In Rsbuild v1, this capability was mainly used to validate compatibility between Rspack and webpack. As Rspack has gradually matured and stabilized, this purpose is no longer necessary, so the related support has been removed.
...
- If you customized Rsbuild logs with the global `logger.override()`, switch to [customLogger](/config/custom-logger) in v2. Instance loggers in v2 are no longer affected by the global `logger.override()`.
