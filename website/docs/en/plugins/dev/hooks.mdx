...
In Rsbuild, some plugin hooks are global hooks. The execution of these hooks is often related to Rsbuild's own startup logic or global logic and is shared under all Environments. Such as:
...
- `onBeforeStartDevServer` and `onAfterStartDevServer` are related to Rsbuild DevServer startup, all Environments share Rsbuild's DevServer, middlewares, and Web Socket.
