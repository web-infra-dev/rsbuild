---
description: 'The React plugin provides support for React, integrating features such as JSX compilation and React Refresh.'
---
...
const fastRefresh = pluginOptions.fastRefresh ?? true;

const defaultOptions = {
  development: isDev,
  refresh: usingHMR && fastRefresh,
  runtime: 'automatic',
};
```
...
const defaultOptions = {
  ...
  include: [
    // Same as the built-in JavaScript rule
  ],
  exclude: [
    // Same as the built-in JavaScript rule
  ],
