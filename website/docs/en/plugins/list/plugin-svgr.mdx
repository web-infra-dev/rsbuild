...
By default, Rsbuild treats SVG files as static assets. For processing rules, refer to: [Static assets](/guide/basic/static-assets).
...
If the imported path doesn't include the `?react` suffix, then the SVG will be treated as a normal static asset and you will get a URL string or base64 URL, see [Static assets](/guide/basic/static-assets).
