...
The current build [target](/config/target).
...
A boolean value indicating whether the build [target](/config/target) is `node`, equivalent to `target === 'node'`.
...
A boolean value indicating whether the build [target](/config/target) is `web-worker`, equivalent to `target === 'web-worker'`.
