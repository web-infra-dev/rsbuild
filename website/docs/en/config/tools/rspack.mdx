---
description: 'The built-in Rspack config in Rsbuild may change with iterations, and these changes will not be reflected in semver.'
---
...
`tools.rspack` accepts an object that gets deep merged with the built-in Rspack configuration through [rspack-merge](https://github.com/rstackjs/rspack-merge).
...
When merging configurations, `rspack-merge` will automatically concatenate arrays such as `plugins`, `module.rules`, `resolve.extensions`, etc.
...
For example, remove the internal ProgressPlugin:
...
Used to merge multiple Rspack configs, the same as [rspack-merge](https://github.com/rstackjs/rspack-merge).
