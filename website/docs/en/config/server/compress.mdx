...
`res.flush()` flushes the gzip buffer without ending the response. It is provided by the compression middleware, so use optional chaining (`?.`) to handle cases where the method is unavailable, such as when compression is disabled.
