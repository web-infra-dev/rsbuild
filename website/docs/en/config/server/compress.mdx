---
description: 'Configure whether to enable gzip compression for static assets served by the dev server or preview server.'
---
...
Gzip compression may buffer data, delaying delivery of streamed HTML. To send a chunk without waiting for the response to end, call `res.flush?.()` after `res.write()`.
...
res.write('<!doctype html><html><body><p>Loading...</p>');
(res as FlushableResponse).flush?.();

// Continue writing as data becomes available, then call res.end().
```

`res.flush()` flushes the gzip buffer without ending the response. It is provided by the compression middleware, so use optional chaining (`?.`) to handle cases where the method is unavailable, such as when compression is disabled. The local type assertion adds `flush` to the response type, since Node.js's types do not declare it.

When using `server.setup`, register your streaming handler with `server.middlewares.use()` inside the returned callback so the compression middleware runs first. See [Middleware execution order](/config/server/setup#middleware-execution-order).
