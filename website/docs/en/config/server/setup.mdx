...
Primarily used to register middleware, and can also run other startup tasks. Similar to [onBeforeStartDevServer](/plugins/dev/hooks#onbeforestartdevserver) and [onBeforeStartPreviewServer](/plugins/dev/hooks#onbeforestartpreviewserver) hooks.
...
It is similar to `onBeforeStartDevServer` and `onBeforeStartPreviewServer`, but configured in `rsbuild.config.ts`.
