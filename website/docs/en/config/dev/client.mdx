...
If the WebSocket URL must be resolved dynamically in the browser, use `dev.client.webSocketUrlResolver` to provide a browser-side module.
...
This is useful when the HMR WebSocket endpoint depends on runtime information, such as a proxy path derived from the current page URL or script URL.
