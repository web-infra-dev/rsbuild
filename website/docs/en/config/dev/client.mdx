...
export default function createWebSocket(url) {
  ...
  // Preserve the token from the original URL for authentication
...
The `url` parameter contains a `token` query parameter used by Rsbuild to identify the environment. You should preserve it when constructing a custom URL.
:::
