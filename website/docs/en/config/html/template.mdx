---
description: 'Specifies the file path for the HTML template, which can be a relative or absolute path.'
---
...
You can also use an async function to load the template path:
  ...
  html: {
    async template({ entryName }) {
      const templates = JSON.parse(
        await fs.readFile('./templates.json', 'utf-8'),
...
| Version | Changes                                              |
| ------- | ---------------------------------------------------- |
| v2.0.8  | Added support for async functions in `html.template` |
