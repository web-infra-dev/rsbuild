---
description: "This option also affects Rsbuild's log output in the browser. You can override this behavior through the client.logLevel option."
---
...
Rsbuild currently cannot set different log levels for its built-in logger per instance because all instances share a global [logger](/api/javascript-api/core#logger).

If you need an isolated logger in custom code or plugins, use [createLogger](/api/javascript-api/core#createlogger) to create a separate logger instance instead of mutating the shared singleton.
