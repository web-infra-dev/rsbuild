...
### Use ASCII for legacy systems

ASCII encoding is useful when deploying to legacy systems or environments with strict character encoding requirements:

```js
// Example: Use ASCII for production builds in legacy environments
const isLegacyEnvironment = process.env.LEGACY_DEPLOYMENT === 'true';

export default {
  output: {
    charset: isLegacyEnvironment ? 'ascii' : 'utf8',
  },
};
```
