...
The `charset` config allows you to specify the [character encoding](https://developer.mozilla.org/en-US/docs/Glossary/Character_encoding) format for output files to ensure they are displayed correctly in different environments.

## UTF8

By default, the output of Rsbuild is encoded in [UTF-8](https://developer.mozilla.org/en-US/docs/Glossary/UTF-8), which is the most commonly used character encoding format in web applications.
...
Currently, the `charset` option only affects files compiled with [SWC](/guide/basic/configure-swc), which by default includes JavaScript and TypeScript files. However, HTML and CSS files are not affected by the `charset` option.
