...
After the production build, Rsbuild adds a hash to the filename by default. To disable this behavior, set [output.filenameHash](/config/output/filename-hash) to `false`.
...
When you import a module via dynamic import, the module will be bundled into a single file using these default naming rules:
...
To specify a fixed name for the async module, use the [magic comments](https://rspack.rs/api/runtime-api/module-methods#magic-comments) that Rspack supports, using `webpackChunkName` to specify the module name:
