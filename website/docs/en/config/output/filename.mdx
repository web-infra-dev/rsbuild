...
After the production build, Rsbuild will add a hash in the middle of the filename by default. If you do not need to add it, you can set [output.filenameHash](/config/output/filename-hash) to `false` to disable this behavior.
...
:::tip Filename hash
Usually, Rsbuild only sets the filename hash in the production mode (i.e., when `process.env.NODE_ENV === 'production'`).

If you set the filename hash in the development mode, it may cause HMR to fail (especially for CSS files). This is because every time the file content changes, the hash value changes, preventing the bundler from reading the latest file content.
:::
