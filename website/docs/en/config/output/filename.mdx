---
description: 'After a production build, Rsbuild adds a hash to the filename by default. To disable this behavior, set output.filenameHash to false.'
---
...
const devDefaultFilename = {
  ...
  wasm: '[contenthash:10].module.wasm',
...
const prodDefaultFilename = {
  html: '[name].html',
  js: output.target === 'node' ? '[name].js' : '[name].[contenthash:10].js',
...
export default {
  output: {
    filename: {
      js: isProd ? '[name]_script.[contenthash:10].js' : '[name]_script.js',
    },
  },
};
```
...
To specify a fixed name for the async module, use the [magic comments](https://rspack.rs/api/runtime-api/module-methods#magic-comments) supported by Rspack. Use `rspackChunkName` to specify the module name:

```js title="src/index.ts"
const { add } = await import(/* rspackChunkName: "my-chunk-name" */ './add.ts');
```
    ...
    filename: {
      js: (pathData, assetInfo) => {
        ...
        if (pathData.chunk?.name === 'index') {
          return isProd ? '[name].[contenthash:10].js' : '[name].js';
        }
  ...
  output: {
    filename: {
      css: (pathData, assetInfo) => {
        if (pathData.chunk?.name === 'index') {
          return isProd ? '[name].[contenthash:10].css' : '[name].css';
        }
...
export default {
  output: {
    filename: {
      js: isProd ? '[name].js?v=[contenthash:10]' : `[name].js`,
      css: isProd ? '[name].css?v=[contenthash:10]' : `[name].css`,
    },
  },
};
```
