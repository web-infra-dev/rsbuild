---
description: 'Configures whether to generate source map files and which source map format to generate.'
---
...
Use `sourceMap.extract` to extract existing source maps from matched JavaScript files. This is useful when a third-party package already ships both `.js` and `.js.map` files.

:::tip
This option is implemented based on Rspack's [`module.rules.extractSourceMap`](https://rspack.rs/config/module-rules#rulesextractsourcemap) and can replace `source-map-loader`.
:::

To enable extraction for all JavaScript files:
...
Use `include` or `exclude` to limit extraction to specific files, which can help reduce performance overhead:
...
| Version | Changes                          |
| ------- | -------------------------------- |
| v2.0.0  | Added `sourceMap.extract` option |
