...
Note that when the value of `externals` is a string, matching is exact. That means you need to explicitly declare subpath imports such as `react-dom/client`.
