...
Note that, with this option on, the style files will no longer be written to the dist directory, they will only exist inside the HTML file instead.
...
After turning on the `output.inlineStyles` option:
