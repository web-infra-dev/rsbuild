...
By default, JavaScript and CSS code are automatically minified in production mode to improve page performance. To disable minification entirely, set `minify` to `false`. You can also control specific minification behavior using the detailed `minify` options:
...
:::tip
This is useful for debugging and troubleshooting. We do not recommend disabling code minification in production builds, as it will significantly impact page performance.
:::
