...
JavaScript and CSS code are automatically minified in production mode to improve page performance. To disable minification entirely, set `minify` to `false`. You can control specific minification behavior using the detailed `minify` options:
