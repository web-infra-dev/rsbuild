...
- **Default:** based on [output.target](/config/output/target):
  ...
  - When `output.target` is `'node'`, the default is `{ preset: 'none', chunks: 'all', minSize: 0 }`. This extracts shared modules into separate chunks to avoid duplicating them across output chunks
