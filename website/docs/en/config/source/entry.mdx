---
description: 'source.entry is an object where the key is the entry name and the value is the path of the entry module or a description object.'
---
...
- **Default:** Rsbuild uses the first existing file in the following order as the `index` entry:
...
If none of these files exist, the default value is `{}`.
