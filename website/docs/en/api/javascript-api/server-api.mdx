...
Rsbuild provides the [server.setup](/config/server/setup) option to access dev and preview server instances.
...
The path should be relative to the dev server root and should not include `server.base`.
