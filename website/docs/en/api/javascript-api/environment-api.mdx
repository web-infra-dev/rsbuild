...
Listen for HMR clients connecting or reconnecting to the current environment. Supports the built-in dev server and custom servers using `connectWebSocket`.
...
The callback runs after a connection is established and any initial build messages are sent. `client.send` accepts the same arguments as `hot.send` and sends only to that client. Sends to closed connections are ignored.

For example, send the latest build revision to each new client:

```ts
// Update after each successful server build.
...
Register the callback before clients connect, such as in `server.setup` or before `server.listen()`. It does not run for existing connections. This API is unavailable with `createDevServer({ runCompile: false })`.

The returned function removes the listener and is safe to call repeatedly. Closing the dev server clears all listeners. Callback errors are logged without interrupting other listeners; returned promises are not awaited.

:::note
A connection does not mean the browser application is ready. Register custom event listeners early and buffer messages until the application can handle them. Rsbuild does not store or replay custom messages.
:::
