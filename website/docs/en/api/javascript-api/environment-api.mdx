...
Register a callback that runs when an HMR client connects or reconnects to the current environment.
...
Register the callback before clients connect, for example in `server.setup` or before calling `server.listen()`. The callback is not invoked for clients that are already connected.

The callback receives a `client` object. Use `client.send` to send messages only to that client, with the same arguments as `hot.send`.

For example, you can send the latest build revision to each client when it connects:

```ts
// Update this value after each successful server build.
...
Call the returned `unsubscribe` function when you no longer need to listen for connections.

:::note
The browser application may still be initializing when the callback runs. Register custom event listeners in the browser as early as possible and buffer messages if needed. Rsbuild does not store or replay custom messages.
:::
