...
Register the callback before clients connect, for example in `server.setup` or before `server.listen()`. Existing connections do not trigger the callback.

Use `client.send` to send messages to that client only. It accepts the same arguments as `hot.send`. For example, send the latest build revision whenever a client connects:
...
// When no longer needed, call unsubscribe() to stop listening.
```

:::note
The browser application may still be initializing when the callback runs. Register custom event listeners early and buffer messages if needed. Rsbuild does not store or replay custom messages.
:::
