...
When `config.customLogger` is provided, `rsbuild.logger` references that logger instance. Otherwise, Rsbuild creates a dedicated logger for the current instance.
