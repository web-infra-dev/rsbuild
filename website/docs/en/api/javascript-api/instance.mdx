...
`rsbuild.logger` is the logger associated with the current Rsbuild instance.

- **Type:** [Logger](/api/javascript-api/core#logger)

When `config.customLogger` is provided, `rsbuild.logger` references that logger instance. Otherwise, it falls back to the shared [logger](/api/javascript-api/core#logger).

This property provides the same logger as [api.logger](/plugins/dev/core#apilogger) in Rsbuild plugins.
