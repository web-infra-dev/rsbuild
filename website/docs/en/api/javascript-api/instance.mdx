...
function preview(options?: PreviewOptions): Promise<StartProdServerResult>;
```
...
- `port`: The actual listening port number.
- `server`: Server instanceServer instance, see [Server API](/api/javascript-api/dev-server-api) for more details.
...
```ts
rsbuild.onBeforeStartProdServer(({ server, environments }) => {
