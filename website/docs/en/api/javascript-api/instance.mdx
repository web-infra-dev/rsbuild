...
You can also expose an API for a specific environment:
...
When a plugin registered in the same environment calls `api.useExposed`, Rsbuild will first resolve the environment-scoped API, then fallback to the global API.
