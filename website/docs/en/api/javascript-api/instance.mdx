...
- `dev`: set when running [rsbuild dev](/guide/basic/cli#rsbuild) or [rsbuild.startDevServer()](/api/javascript-api/instance#rsbuildstartdevserver)
...
`rsbuild.logger` is the logger associated with the current Rsbuild instance. See [Logging](/guide/advanced/logging) for more details.
...
- `server`: Server instance, see [Server API](/api/javascript-api/server-api) for more details.
...
Call the `server.close()` method to close the dev server, trigger the [onCloseDevServer](/plugins/dev/hooks#onclosedevserver) hook, and perform cleanup operations.
...
- To integrate the Rsbuild dev server into a custom server, you can use the `createDevServer` method to create a dev server instance. Refer to [Server API](/api/javascript-api/server-api) for all available APIs.
...
:::tip
To view the Rsbuild and Rspack configurations during the build process, use [debug mode](/guide/debug/debug-mode), or obtain them through hooks such as [onBeforeBuild](#rsbuildonbeforebuild), [onBeforeCreateCompiler](#rsbuildonbeforecreatecompiler).
:::
...
> Provides the same functionality as the [onBeforeStartPreviewServer](/plugins/dev/hooks#onbeforestartpreviewserver) plugin hook.
...
> Provides the same functionality as the [onAfterStartPreviewServer](/plugins/dev/hooks#onafterstartpreviewserver) plugin hook.
