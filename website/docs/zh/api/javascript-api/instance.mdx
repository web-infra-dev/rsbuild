...
一些基于 Rsbuild 的工具已经设置了 `callerName` 选项：
...
function Build(options?: BuildOptions): Promise<{
  /**
   * Rspack 的 [stats](https://rspack.dev/zh/api/javascript-api/stats) 对象。
   */
...
在非 watch 模式下，`rsbuild.build()` 会返回一个 Rspack 的 [stats](https://rspack.dev/zh/api/javascript-api/stats) 对象。
...
创建一个 Rspack [Compiler](https://rspack.dev/zh/api/javascript-api/compiler) 实例；如果本次构建存在多个 [environments](/config/environments)，则返回值为 [MultiCompiler](https://rspack.dev/zh/api/javascript-api/compiler#multicompiler)。
