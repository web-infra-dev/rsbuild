...
`rsbuild.logger` 表示当前 Rsbuild 实例所关联的 logger。

- **类型：** [Logger](/api/javascript-api/core#logger)

当配置了 `config.customLogger` 时，`rsbuild.logger` 会引用该 logger 实例；否则会回退到共享的 [logger](/api/javascript-api/core#logger)。

该属性与 Rsbuild 插件中的 [api.logger](/plugins/dev/core#apilogger) 指向同一个 logger。
