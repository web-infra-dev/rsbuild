...
监听 HMR 客户端连接或重新连接到当前 environment。支持内置 dev server 和通过 `connectWebSocket` 接入的自定义服务器。
...
回调在连接建立并发送已有的初始构建消息后触发。`client.send` 接受与 `hot.send` 相同的参数，只向该客户端发送消息。向已关闭的连接发送消息会被忽略。

例如，向每个新连接的客户端发送最新的构建版本：
...
请在客户端连接前注册回调，例如在 `server.setup` 中或调用 `server.listen()` 之前。已有连接不会触发该回调。使用 `createDevServer({ runCompile: false })` 时，此 API 不可用。

返回的函数用于移除监听器，可安全地重复调用。关闭 dev server 时会清理所有监听器。回调中的错误会被记录，不会中断其他监听器；返回的 Promise 不会被等待。

:::note
连接建立不代表浏览器端应用已就绪。请尽早注册自定义事件监听器，并在应用能够处理消息前缓存消息。Rsbuild 不会保存或重放自定义消息。
:::
