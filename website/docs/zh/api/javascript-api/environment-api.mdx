...
注册回调，在 HMR 客户端连接到当前 environment 时触发，包括断线重连。
...
使用 `client.send` 向该客户端单独发送消息，参数与 `hot.send` 相同。例如，在客户端连接时发送最新的构建版本：
...
// 不再需要监听时，调用 unsubscribe() 取消监听。
```

:::note
回调触发时，浏览器端应用可能仍在初始化。请尽早注册自定义事件监听器，并按需缓存消息。Rsbuild 不会保存或重放自定义消息。
:::
