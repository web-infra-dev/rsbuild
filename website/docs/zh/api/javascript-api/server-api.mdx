---
description: 'Rsbuild JavaScript API 参考，涵盖如何使用、示例、共享 API等核心接口和用法。'
---
...
Rsbuild 提供了 [server.setup](/config/server/setup) 选项，可以访问 dev server 和 preview server 实例。
...
- 如果启用了 [server.https](/config/server/https)，则为 `Http2SecureServer`。
- 如果启用了 [server.middlewareMode](/config/server/middleware-mode)，则为 `null`。
...
默认从 [server.port](/config/server/port) 开始，如果端口被占用会自动递增并使用可用端口。
...
使用 [server.middlewareMode](/config/server/middleware-mode) 时不需要调用该方法。
...
这个路径应当相对于 dev server 根路径，并且不应包含 `server.base`。
