...
在本地开发场景，建议使用 [rsbuild.startDevServer](/api/javascript-api/instance#rsbuild-startdevserver) 方法，调用后会启动本地 Dev Server。
...
在生产环境部署场景，建议使用 [rsbuild.build](/api/javascript-api/instance#rsbuild-build) 方法，调用后会构建出生产环境产物。
