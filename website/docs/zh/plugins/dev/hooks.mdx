---
description: '本章节介绍 Rsbuild 插件可用的 hooks。'
---

# 插件 hooks \{#plugin-hooks}

本章节介绍 Rsbuild 插件可用的 hooks。
...
- [onBeforeStartPreviewServer](#onbeforestartpreviewserver)：在启动预览服务器前调用。
- [onAfterStartPreviewServer](#onafterstartpreviewserver)：在启动预览服务器后调用。
...
:::warning
`modifyRsbuildConfig` 为全局 hook。如果你希望你开发的插件支持[仅在特定的 environment 下生效](/guide/advanced/environments#plugins-specified-environment)，应避免使用 `modifyRsbuildConfig`，可使用 [modifyEnvironmentConfig](/plugins/dev/hooks#modifyenvironmentconfig) 代替。
:::
...
:::tip
`modifyRsbuildConfig` 不能用于注册额外的 Rsbuild 插件。这是因为在执行 `modifyRsbuildConfig` 时，Rsbuild 已经初始化了所有插件，并开始执行 hooks 的回调函数。详情可参考 [插件注册时机](/config/plugins#plugin-registration-phase)。
:::
...
回调函数的第二个参数 `utils` 是一个对象，包含了一些工具函数和属性，详见 [tools.rspack - 工具对象](/config/tools/rspack#utils)。
...
function ModifyBundlerChain(
  callback: (chain: RspackChain, utils: ModifyBundlerChainUtils) => Promise<void> | void,
): void;
```
...
const myPlugin = () => ({
  setup(api) {
    api.modifyBundlerChain((chain, utils) => {
      ...
      chain.plugin('circular-dependency').use(utils.rspack.CircularDependencyRspackPlugin);
    });
  },
});
```

回调函数的第二个参数 `utils` 是一个对象，包含了一些工具函数和属性，详见 [tools.bundlerChain - 工具对象](/config/tools/bundler-chain#utils)。
...
function ModifyHTML(callback: (html: string, context: Context) => MaybePromise<string>): void;
```
...
查看 [html.tags](/config/html/tags) 了解如何定义标签。
