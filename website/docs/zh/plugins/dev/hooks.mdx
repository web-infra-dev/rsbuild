...
- [onDevCompileEnvironmentDone](#ondevcompileenvironmentdone): 在每次单个 environment 的开发环境构建结束后调用。
...
- [onBeforeEnvironmentBuild](#onbeforeenvironmentbuild): 在执行单个 environment 的生产环境构建前调用。
- [onBeforeBuild](#onbeforebuild)：在执行生产环境构建前调用。
- [onAfterEnvironmentBuild](#onafterenvironmentbuild): 在执行单个 environment 的生产环境构建后调用，可以获取到单个 environment 的构建结果信息。
...
import OnBeforeEnvironmentBuild from '@zh/shared/onBeforeEnvironmentBuild.mdx';
...
import OnAfterEnvironmentBuild from '@zh/shared/onAfterEnvironmentBuild.mdx';
...
import OnDevCompileEnvironmentDone from '@zh/shared/onDevCompileEnvironmentDone.mdx';
