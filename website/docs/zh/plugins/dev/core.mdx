...
- `apply`: 控制插件在 `serve` 或 `build` 时生效，详见 [条件启用](#条件启用)。
- `enforce`: 指定插件的执行顺序，详见 [enforce 属性](#enforce-属性)。
- `pre`：声明前置插件的名称，这些插件会在当前插件之前执行，详见 [前置插件](#前置插件)。
- `post`：声明后置插件的名称，这些插件会在当前插件之后执行，详见 [后置插件](#后置插件)。
- `remove`：声明需要移除的插件，可以传入插件 name 的数组，详见 [移除插件](#移除插件)。
...
你可以在 `transform` 函数中返回一个 source map，Rsbuild 会自动将返回的 source map 与其他 Rspack loader 或 `transform` hook 生成的 source map 合并，使最终的 source map 能够正确映射到原始源代码。
