...
1. environment 配置不等于 rsbuild 配置：
   - 使用 [environment 上下文](/guide/advanced/environments#environment-上下文) 代替 `getRsbuildConfig` 获取 environment 信息。
   - 修改特定 environment 配置时，使用 [modifyEnvironmentConfig](/plugins/dev/hooks#modifyenvironmentconfig) 代替 [modifyRsbuildConfig](/plugins/dev/hooks#modifyrsbuildconfig) (避免对其他环境产生影响)。
