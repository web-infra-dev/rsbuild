---
description: '使用 Sass 作为 CSS 预处理器，基于 sass-loader 实现。'
---
...
注册后，即可在代码中引入 `*.scss`，`*.sass`，`*.module.scss` 或 `*.module.sass` 文件，无须添加其他配置。
...
```js
const rewriteUrls = pluginOptions.rewriteUrls ?? true;
...
const defaultOptions = {
  ...
  sourceMap: rewriteUrls ? true : useCssSourceMap,
  sassOptions: {
    quietDeps: true,
