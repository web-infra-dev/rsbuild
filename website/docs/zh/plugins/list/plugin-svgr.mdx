---
description: '默认情况下，Rsbuild 会将 SVG 图片当作静态资源处理，处理规则可参考：静态资源。'
---
...
注册后，在 **JS 文件**中引用带有 `?react` 后缀的 SVG 资源时，Rsbuild 会调用 SVGR 将 SVG 图片转换为 React 组件。
...
type PluginSvgrOptions = {
  ...
  /**
   * 排除一部分模块，这些模块引用的 SVG 文件不会经过 SVGR 处理
   */
