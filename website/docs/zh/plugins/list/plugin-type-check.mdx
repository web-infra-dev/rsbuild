...
插件在开发模式和生产模式下的行为有以下差异：

- 在开发模式构建时，类型错误不会阻塞构建流程，只会在 terminal 中输出错误日志。
- 在生产模式构建时，类型错误会导致构建失败，以保证生产模式代码的稳定性。
...
仅在开发模式构建时开启类型检查（不建议在生产模式构建时禁用类型检查，这会导致生产代码的稳定性下降）：
...
`exclude` 选项可以通过基于 TS 错误的 `code`、`message` 或 `file` 进行过滤。

例如类型不匹配的错误，可以通过 `code: 'TS2345'` 来排除：

```ts
pluginTypeCheck({
  forkTsCheckerOptions: {
    issue: {
      // 忽略 "Argument of type 'string' is not assignable to parameter of type 'number'.ts(2345)"
      exclude: [{ code: 'TS2345' }],
    },
  },
});
...
或是通过 `file` 来排除 `/some-folder/` 下的文件：
...
3. 由于生产构建使用 `vue-tsc` 进行类型检查，你可以在生产模式关闭 Type Check 插件，避免重复执行检查。
