...
```ts
type Prefetch = undefined | true | PrefetchOptions | PrefetchOptions[];
...
`performance.prefetch` 可以设置为一个对象或对象数组来指定选项。
...
你也可以将 `performance.prefetch` 设置为数组来应用多条 prefetch 规则，匹配到的资源会被合并并去重：
