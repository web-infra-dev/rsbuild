---
description: '为 Rsbuild 构建生成的静态资源注入 link 标签。'
---
...
```ts
type Preload = undefined | true | PreloadOptions | PreloadOptions[];
...
`performance.preload` 可以设置为一个对象或对象数组来指定选项。
...
```ts
type ResourceHintsIncludeType = 'async-chunks' | 'initial' | 'all-assets' | 'all-chunks';
```
...
你也可以将 `performance.preload` 设置为数组来应用多条 preload 规则，匹配到的资源会被合并并去重：
...
设置为 `false` 时，Rsbuild 将为所有符合条件的资源生成 preload 标签，即使这些资源已经在 HTML 中存在。
