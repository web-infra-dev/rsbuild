---
description: 'server.open 用于配置一组页面 URL，Rsbuild 会在启动 server 后自动在浏览器中打开这些页面。'
---
...
> 你也可以使用 Rsbuild CLI 的 [--open](/guide/basic/cli#opening-page) 选项来打开页面。当同时使用 `server.open` 和 `--open` 时，`--open` 的优先级更高。
...
```bash
# Chrome
BROWSER=chrome npx rsbuild

# Edge
BROWSER=edge npx rsbuild

# Firefox
BROWSER=firefox npx rsbuild
```
...
```bash
npm i cross-env -D
cross-env BROWSER=chrome npx rsbuild
```
...
```bash
# macOS
BROWSER="google chrome" npx rsbuild
...
```bash
BROWSER=chrome BROWSER_ARGS="--incognito" npx rsbuild
```
...
```bash
BROWSER="Google Chrome Canary" npx rsbuild
```

### 环境变量配置

你可以将 `BROWSER` 设置在本地的 [.env.local](/guide/advanced/env-vars#env-file) 文件中，这样能够避免每次启动 dev server 时都需要手动设置环境变量，同时也能避免影响项目的其他开发者。
