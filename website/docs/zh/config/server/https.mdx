---
description: '配置 HTTPS 选项以启用 HTTPS 服务器。当启用时，HTTP 服务器将被禁用。'
---
...
:::tip
当启用 HTTPS 时，Rsbuild 默认会启用 HTTP/2。
:::
...
对基本的配置需求，你可以添加 [@rsbuild/plugin-basic-ssl](https://github.com/rstackjs/rsbuild-plugin-basic-ssl) 插件，它会自动创建一个自签名的证书，并默认设置 `server.https` 选项。
