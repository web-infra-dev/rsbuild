---
description: '为 HTML 所引入的脚本资源添加随机属性值 nonce，使浏览器在解析到带有匹配 nonce 值的内联脚本时，能判断该脚本是否能执行。'
---
...
nonce 机制在 Content Security Policy（CSP，内容安全策略）中起到关键作用，用于提升网页安全性。它允许开发者在 CSP 中为内联 `<script>` 标签和 `<style>` 标签定义一个唯一且随机的字符串值，即 `nonce`。
...
通常可以在项目中定义一个固定值，并在 Nginx、Web Server、网关等下游服务器的响应中统一替换成随机值。
...
- 由 Rspack 动态生成的 `<script>` 标签（通过 [`import.meta.rspackNonce`](https://rspack.rs/zh/api/runtime-api/module-variables#importmetarspacknonce) 变量实现）
- 通过 [`html.tags`](/config/html/tags) 添加的支持 nonce 的标签，可通过 `attrs.nonce` 覆盖默认值，或设为 `false` 不添加该属性
