---
description: '该值会被 SWC 和 Lightning CSS 等工具读取，用来确定需要转换的 JavaScript 语法特性和需要添加的 CSS 浏览器前缀。'
---
...
该值会被 [SWC](https://github.com/swc-project/swc) 和 [Lightning CSS](https://github.com/parcel-bundler/lightningcss) 等工具读取，用来确定需要转换的 JavaScript 语法特性和需要添加的 CSS 浏览器前缀。
...
如果项目中没有定义任何 `browserslist` 相关的配置，也没有定义 `overrideBrowserslist`，那么 Rsbuild 会使用 [默认值](/guide/advanced/browserslist#default-values)。
