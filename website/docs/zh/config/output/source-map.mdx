---
description: '用于设置是否生成 source map 文件，以及生成哪一种格式的 source map。'
---
...
使用 `sourceMap.extract` 可以从匹配的 JavaScript 文件中提取已有的 source map。这在第三方包已经同时产出 `.js` 和 `.js.map` 文件时会很有用。

:::tip
该功能基于 Rspack 的 [`module.rules.extractSourceMap`](https://rspack.rs/config/module-rules#rulesextractsourcemap) 实现，可以替代 `source-map-loader`。
:::
