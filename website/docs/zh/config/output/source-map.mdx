...
使用 `sourceMap.extract` 可以从匹配的 JavaScript 文件中提取已有的 source map。这在某个包已经同时产出 `.js` 和 `.js.map` 文件时会很有用。

要为内置 JS 规则匹配到的所有文件开启提取，可以这样配置：
...
你也可以通过 `include` 或 `exclude` 将提取范围限制到特定文件：
