---
description: '设置构建产物的输出目录，Rsbuild 会根据产物的类型将其输出到相应的子目录中。'
---
...
- `string`：设置根输出目录到指定路径，等同于 `distPath.root`。
- `object`：为每种文件类型设置输出目录。
...
- `image`：非 SVG 图片的输出目录。
- `media`：视频等媒体资源的输出目录。
- `assets`：其他静态资源的输出目录。例如 [扩展静态资源类型](/guide/basic/static-assets#extend-asset-types) 中定义的资源。
...
| 版本    | 变更内容                             |
| ------- | ------------------------------------ |
| v1.4.13 | 新增对 `distPath.favicon` 选项的支持 |
| v1.6.0  | 新增对 `string` 类型的支持           |
