---
description: '用来控制是否用 script 标签将产物中的 script 文件（.js 文件）inline 到 HTML 中。'
---
...
```ts
type InlineChunkTestFunction = (params: { size: number; name: string }) => boolean;
...
如果你希望将异步 Chunks 内联到 HTML 中，你可以通过 [tools.rspack](/config/tools/rspack) 配置项来实现修改 Rspack 的默认行为，将 [module.parser.javascript.dynamicImportMode](https://rspack.rs/zh/config/module-parser#javascriptdynamicimportmode) 设置为 `'eager'`，此时 Rspack 不会为 dynamic import 生成单独的 JS 文件。
