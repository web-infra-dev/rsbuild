...
- 将 Rspack 的 [target](https://rspack.dev/zh/config/target) 设置为 `'node'`。
...
- 将 Rspack 的 [target](https://rspack.dev/zh/config/target) 设置为 `'webworker'`。
...
Rspack 支持的 [target](https://rspack.dev/zh/config/target) 类型更为丰富，比如 `electron-main` 和 `electron-renderer` 等。
