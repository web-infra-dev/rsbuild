---
description: '指定 Rsbuild 构建的模式，各个模式默认的行为和优化不同，比如 production 模式默认会压缩代码。'
---
...
Rsbuild CLI 支持使用 `--env-mode` 选项来指定 env 模式，详见 ["Env 模式"](/guide/advanced/env-vars#env-mode)。
:::
...
- `rsbuild` 会将 `NODE_ENV` 和 `mode` 的默认值设置为 `development`。
