---
description: '默认情况下，插件会按照 plugins 数组中的顺序依次执行，Rsbuild 内置插件的执行时机早于用户注册的插件。'
---
...
type RsbuildPlugins = (
  RsbuildPlugin | Falsy | Promise<RsbuildPlugin | Falsy | RsbuildPlugins> | RsbuildPlugins
)[];
```
...
如果插件使用了控制执行顺序的字段，比如 `enforce` 属性时，则最终的执行顺序会根据这些字段进行调整，详见 [`enforce` 属性](/plugins/dev/core#enforce-property)。
...
## 插件注册时机 \{#plugin-registration-phase}
...
:::tip
在使用 unplugin 的 transform hook 时，请使用 `transformInclude` hook 来匹配指定的模块。当 transform hook 会匹配到 `.html` 模块时，会代替 [html-rspack-plugin](https://github.com/rstackjs/html-rspack-plugin) 默认的 EJS 转换。
:::
