---
description: 'source.entry 的值是一个对象，key 是入口名称，value 是入口模块的路径，或是一个 描述对象。'
---
...
```ts
type Entry = Record<string, string | string[] | (Rspack.EntryDescription & { html?: boolean })>;
```

- **默认值：** Rsbuild 会按照以下顺序查找文件，并将第一个存在的文件作为 `index` 入口：
...
`source.entry` 的值是一个对象，key 是入口名称，value 是入口模块的路径，或是一个 [描述对象](#description-object)。

当 value 为路径时，它可以为绝对路径或相对路径，相对路径会基于 [root](/config/root) 进行解析。

## HTML 生成 \{#html-generation}

Rsbuild 会为 `source.entry` 中的每一个入口注册 [html-rspack-plugin](https://github.com/rstackjs/html-rspack-plugin)，并生成对应的 HTML 文件。
...
- 将入口描述对象中的 [html](/config/source/entry#html-property) 属性设置为 `false` 来禁用单个入口的 HTML 生成。
- 将 [tools.htmlPlugin](/config/tools/html-plugin) 设置为 `false` 来禁用所有入口的 HTML 生成。

## 描述对象 \{#description-object}
