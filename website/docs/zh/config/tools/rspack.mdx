---
description: 'tools.rspack 选项用于修改 Rspack 的配置项。'
---
...
`tools.rspack` 可以配置为一个对象，这个对象将会和内置的 Rspack 配置通过 [rspack-merge](https://github.com/rstackjs/rspack-merge) 进行深层合并。
...
在合并配置时，`rspack-merge` 会自动合并数组，比如 `plugins`、`module.rules`、`resolve.extensions` 等配置。
...
用于合并多份 Rspack 配置，等价于 [rspack-merge](https://github.com/rstackjs/rspack-merge)。
