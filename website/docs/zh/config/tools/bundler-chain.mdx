---
description: 'Rsbuild 的 tools.bundler-chain 配置项参考，包含作用、默认值、配置行为和使用示例。'
---
...
```ts
type BundlerChainFn = (chain: RspackChain, utils: ModifyBundlerChainUtils) => Promise<void> | void;
```
...
参考：[RspackChain 示例](/guide/configuration/rspack#use-bundler-chain)。
...
详见 [自定义 loader](/guide/configuration/rspack#custom-loader)

### CHAIN_ID.PLUGIN

通过 `PLUGIN.[ID]` 可以匹配到特定的 Rspack 或 webpack plugin。

详见 [自定义 Plugin](/guide/configuration/rspack#custom-plugin)
