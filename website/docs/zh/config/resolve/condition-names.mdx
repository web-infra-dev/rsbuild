---
description: '指定用于匹配包 exports 字段 入口点的 condition names（条件名称）。'
---
...
`tools.rspack` 基于 [rspack-merge](https://github.com/rstackjs/rspack-merge) 来合并配置中的数组，这意味着 `tools.rspack.resolve.conditionNames` 会与 Rsbuild 的默认值合并，而不是覆盖它。
