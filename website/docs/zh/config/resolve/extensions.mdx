---
description: '自动添加导入文件的扩展名。这意味着你可以导入文件，而不需要显式地写它们的扩展名。'
---
...
`tools.rspack` 基于 [rspack-merge](https://github.com/rstackjs/rspack-merge) 来合并配置中的数组，这意味着 `tools.rspack.resolve.extensions` 会与 Rsbuild 的默认值合并，而不是覆盖它。
