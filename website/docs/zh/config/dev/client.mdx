---
description: '配置 Rsbuild 在开发过程中注入的 client 代码，可以用于设置热更新对应的 WebSocket URL。'
---
...
自定义 WebSocket transport 模块的路径。该模块需要 default export 一个工厂函数，签名为 `(url: string) => WebSocket`。函数接收 Rsbuild 计算的 WebSocket URL，需要返回一个标准的 `WebSocket` 实例。

这在微前端或反向代理场景下非常有用——当页面 origin 和 dev server origin 不同时，可以通过自定义 transport 来重写 WebSocket URL 的解析逻辑。
...
以下是一个自定义 transport 模块的示例，它从脚本自身的 URL 推导 WebSocket 地址：
...
:::tip
工厂函数在每次建立 WebSocket 连接时都会被调用，包括断线重连。由于 `document.currentScript` 仅在脚本同步执行期间可用，你需要在首次调用时缓存计算结果（如上面的示例所示）。
:::
