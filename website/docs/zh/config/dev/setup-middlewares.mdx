...
提供 Rsbuild 的 [environment API](/api/javascript-api/environment-api#environment-api)，详见 [Server API - environments](/api/javascript-api/server-api#environments)。
...
向 HMR 客户端传递一些消息，详见 [Server API - sockWrite](/api/javascript-api/server-api#sockwrite)。
