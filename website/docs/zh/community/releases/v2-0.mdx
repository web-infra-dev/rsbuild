...
[server.host](/config/server/host) 的默认值从 `'0.0.0.0'` 调整为 `'localhost'`。这意味着开发服务器默认仅监听本机，不再对局域网内的其他设备开放。
...
开发服务器使用的 Proxy 中间件已升级至最新版本。[http-proxy-middleware](https://github.com/chimurai/http-proxy-middleware) 由 v2 升级到了 v4，同时其底层依赖从已经停止维护的 `http-proxy` 切换为由 unjs 社区积极维护的 [httpxy](https://npmx.dev/package/httpxy)。
