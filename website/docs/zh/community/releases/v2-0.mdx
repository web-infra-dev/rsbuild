...
- 更轻量：
  - [依赖数量从 14 个减少到 5 个，安装体积减少 2 MB](#精简依赖)
...
- 不再默认安装 `core-js`，在启用 [output.polyfill](/config/output/polyfill) 时需要手动安装。
- 不再默认安装 `@module-federation/runtime-tools` 不再默认安装，在通过 [moduleFederation.options](/config/module-federation/options) 时需要手动安装，Module Federation 2.0 不受影响。
