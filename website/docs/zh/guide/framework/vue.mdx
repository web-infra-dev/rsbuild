---
description: '在这篇文档中，你可以了解到如何基于 Rsbuild 构建 Vue 应用，以及如何支持 Vue 2。'
---

# Vue

在这篇文档中，你可以了解到如何基于 Rsbuild 构建 Vue 应用，以及如何支持 Vue 2。

## 创建 Vue 应用

使用 [create-rsbuild](/guide/start/quick-start#create-an-rsbuild-application) 来创建一个基于 Rsbuild 的 Vue 应用，运行以下命令：
...
然后在 `Select framework` 时选择 `Vue` 即可。
...
如果你需要使用 Vue 的 JSX 语法，还需要注册 Rsbuild 的 [Vue JSX 插件](https://github.com/rstackjs/rsbuild-plugin-vue-jsx)。
...
为了能够编译 Vue 的 SFC（单文件组件），你需要注册 Rsbuild 的 [Vue 2 插件](https://github.com/rstackjs/rsbuild-plugin-vue2)，插件会自动添加 Vue 构建所需的配置。
...
如果你需要使用 Vue 的 JSX 语法，还需要注册 Rsbuild 的 [Vue 2 JSX 插件](https://github.com/rstackjs/rsbuild-plugin-vue2-jsx)。
