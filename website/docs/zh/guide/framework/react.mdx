...
- 推荐：通过 [@rsbuild/plugin-react](/plugins/list/plugin-react#reactcompiler) 启用 Rust 版本的 React Compiler。该方案使用 Rspack 内置的 SWC loader，配置更简单，构建性能更好。
...
如果你使用 Rsbuild 2.1.0 或更高版本，可以直接通过 `@rsbuild/plugin-react` 启用 Rust 版本的 React Compiler：
...
将 `pluginReact` 中的 `reactCompiler` 设置为 `true`，即可启用 Rust 版本的 React Compiler：
...
该选项通过 Rspack 内置的 SWC loader 启用 React Compiler。请参考[更多详情](/plugins/list/plugin-react#reactcompiler)。
