---
description: '进行构建性能分析可以帮助你确定项目中的性能瓶颈，从而采取针对性的优化。'
---
...
默认情况下，Rspack 会通过 `logger` trace layer 将 profile 输出到 stdout。

如果需要生成 Perfetto trace 文件，可以设置 `RSPACK_TRACE_LAYER=perfetto`。Perfetto tracing 只在 Rspack debug 包中可用，release 包不会启用该能力。启用 Perfetto tracing 后，Rsbuild 会在当前目录下生成一个 `.rspack-profile-${timestamp}-${pid}` 文件夹，其中包含 `rspack.pftrace` 文件（在 1.4.0 之前的版本中为 `trace.json`），该文件由 Rspack 基于 [tracing](https://github.com/tokio-rs/tracing) 细粒度地记录了各个阶段的耗时，可以使用 [ui.perfetto.dev](https://ui.perfetto.dev/) 进行查看。

:::tip

- 在关闭 dev server 时，优先使用 `CTRL + D` 关闭，而不是 `CTRL + C`，这可以确保 Rspack 能够完整地记录性能数据。
- 生成的 profile 输出体积可能较大，如果需要分享给他人查看，可以将 profile 文件压缩为 zip 文件来减小体积。
