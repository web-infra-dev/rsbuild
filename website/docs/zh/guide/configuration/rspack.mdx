---
description: 'Rsbuild 支持直接修改 Rspack 配置对象，也支持通过 rspack-chain 来修改 Rsbuild 内置的 Rspack 配置。'
---
...
## 使用 Bundler chain \{#use-bundler-chain}
...
- 第二个参数为一个[工具对象](/config/tools/bundler-chain#utils)，包括 `env`、`isProd`、`CHAIN_ID` 等。
...
以上是一些常见的配置示例，完整的 API 请见 [rspack-chain 文档](https://github.com/rstackjs/rspack-chain)。
...
- [tools.bundlerChain](/config/tools/bundler-chain)
