...
升级 Rsbuild 到最新版本，可以获得最新的性能优化，参考 [升级 Rsbuild](/guide/upgrade/upgrade-rsbuild) 章节。
...
如果项目中使用了 React Compiler，建议通过 [Rsbuild 内置的 Rust 版本的 React Compiler](/plugins/list/plugin-react#reactcompiler) 来启用，而不是通过 Babel 接入 React Compiler，从而减少 Babel 带来的性能开销。
...
- [@rsbuild/plugin-babel - parallel](/plugins/list/plugin-babel#parallel)：并行执行 Babel 转换。
- [@rsbuild/plugin-less - parallel](/plugins/list/plugin-less#parallel)：并行编译 Less 模块。
- [@rsbuild/plugin-svgr - parallel](/plugins/list/plugin-svgr#parallel)：并行将 SVG 模块转换为 React 组件。
...
参考 [Tailwind CSS v3 - 优化构建性能](/guide/styling/tailwindcss-v3#optimize-build-performance) 了解更多。
