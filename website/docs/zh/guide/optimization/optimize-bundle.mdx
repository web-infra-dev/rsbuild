...
- 如果你在使用 `pnpm < 7.26.0` 版本，可以使用 [pnpm-deduplicate](https://github.com/ocavue/pnpm-deduplicate) 来分析出所有的重复依赖，并通过升级依赖或声明 [pnpm overrides](https://pnpm.io/settings#overrides) 进行版本合并。
...
## 提升 Browserslist 范围 \{#adjust-browserslist}
...
合理的代码分割策略有助于提升应用的加载性能。通过将代码分割为合适的 chunk，可以更好地利用浏览器缓存，减少不必要的请求，从而改善页面的加载效率。

参考 [代码分割](/guide/optimization/code-splitting) 了解如何进行代码分割。
