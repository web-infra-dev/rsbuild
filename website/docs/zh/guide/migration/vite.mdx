...
Rsbuild 不支持 Vite 的 `?worker&url`、`?sharedworker`、`?sharedworker&inline` 或 `?sharedworker&url` query 后缀。如果你只是使用这些 query 来创建 worker，可以迁移为更标准的构造器语法：
...
构造器语法也允许你传入标准的 `WorkerOptions`，例如 `name`、`type` 和 `credentials`。更多用法请参考 [Web Workers](/guide/basic/web-workers)。
