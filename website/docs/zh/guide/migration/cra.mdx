...
值得注意的是，CRA 允许在代码中访问完整的 `process.env` 对象，也允许对 `process.env` 进行解构。
...
而 Rsbuild 出于包体积和安全的考虑，并没有注入完整的 `process.env` 对象，但你可以通过 [source.define](/config/source/define) 配置项来手动注入：
