...
CRA 项目在 eject 后，更近似于直接使用 webpack 的项目，因此可参考 [webpack 迁移指南](/guide/migration/webpack)。

:::

## 使用 Agent Skills

使用支持 Skills 的 coding agent 时，可安装 [migrate-cra-to-rsbuild](https://github.com/rstackjs/agent-skills#migrate-cra-to-rsbuild) 技能来辅助完成迁移流程。
...
:::tip
Rsbuild 没有内置测试框架，因此没有提供用于替换 `react-scripts test` 的命令，可直接使用 [Rstest](https://github.com/web-infra-dev/rstest)、Jest 或 Vitest 等测试框架。查看 [测试](/guide/advanced/testing) 了解更多。
:::
...
CRA 默认使用 `public/index.html` 文件作为 HTML 模板。在 Rsbuild 中，通过 [html.template](/config/html/template) 指定 HTML 模板：
...
配置 Rsbuild 的 [output.distPath.root](/config/output/dist-path) 选项，将目录修改为 `build`，与 CRA 保持一致：
...
如果你需要处理 node_modules 中依赖引起的语法兼容性问题，可以使用 [source.include](/config/source/include#compile-node_modules) 配置项来编译 node_modules。
...
CRA 默认会将 `REACT_APP_` 开头的环境变量注入到 client 代码中，而 Rsbuild 默认会注入 `PUBLIC_` 开头的环境变量（参考 [public 变量](/guide/advanced/env-vars#public-variables)）。

为了兼容 CRA 的行为，可手动调用 Rsbuild 提供的 [loadEnv](/api/javascript-api/core#loadenv) 方法来读取 `REACT_APP_` 开头的环境变量，并通过 [source.define](/config/source/define) 配置项注入到 client 代码中。
...
在 Rsbuild 中，可通过 [source.define](/config/source/define) 配置项设置 `process.env`，读取 `loadEnv` 方法返回的 `rawPublicVars`，来允许以上用法：
...
在 Rsbuild 项目中，可移除 react-app-polyfill 相关的依赖和代码，因为 Rsbuild 会自动读取 browserslist 配置，并允许通过 [output.polyfill](/config/output/polyfill) 配置项启用 polyfill 注入。
