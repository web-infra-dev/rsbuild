...
以下 Dockerfile 适用于 npm 项目。使用其他包管理器时，请复制对应的 lockfile，并改用其不可变的生产依赖安装命令。
...
运行 dev server、生产构建、测试套件和生产 server。移除 Vite 前，请确认所有测试工具、插件和其他依赖均可在没有 Vite 的情况下工作：
...
如果某个工具仍会解析或导入 Vite，请在移除 Vite 前迁移或更新该工具。某个包声明 Vite 为 optional peer 并不意味着 runtime 必须依赖 Vite；请验证实际应用的构建和测试。
