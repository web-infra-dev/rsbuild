---
description: 'Rsbuild 内置了一个轻量的命令行工具，包含 rsbuild dev、rsbuild build 等命令。'
---

# 命令行工具

Rsbuild 内置了一个轻量的命令行工具，包含 [rsbuild dev](#rsbuild)、[rsbuild build](#rsbuild-build) 等命令。
...
| `-c, --config <config>`    | 指定配置文件路径（相对路径或绝对路径），详见 [指定配置文件](/guide/configuration/rsbuild#指定配置文件)               |
| `--config-loader <loader>` | 指定配置文件加载方式（`auto` \| `jiti` \| `native`），详见 [指定加载方式](/guide/configuration/rsbuild#指定加载方式) |
...
`rsbuild` 命令用于启动本地开发服务器，并以开发模式编译源代码。`rsbuild dev` 是 `rsbuild` 的别名。
...
### 指定 Host \{#specifying-host}

默认情况下，dev server 仅监听 `localhost`。当传入 `--host` 且未指定值时，服务器会监听所有网络接口：
