---
description: 'Rsbuild 默认支持 TypeScript，你可以直接在项目中使用 .ts 和 .tsx 文件。'
---
...
## TypeScript 转换 \{#typescript-transformation}
...
要启用类型检查，可以使用 [@rsbuild/plugin-type-check](https://github.com/rstackjs/rsbuild-plugin-type-check) 插件。该插件会在单独的进程中运行 TypeScript 类型检查，并在内部集成了 [ts-checker-rspack-plugin](https://github.com/rstackjs/ts-checker-rspack-plugin)。

该插件支持在开发模式和构建模式下进行类型检查，帮助你在开发过程中及时发现类型错误。

请参考 [@rsbuild/plugin-type-check](https://github.com/rstackjs/rsbuild-plugin-type-check) 了解用法。
...
默认情况下，Rsbuild 会使用 `2023-11` 版本的装饰器，你也可以按需将其设置为 `legacy` 或其他版本：
