---
description: 'Rsbuild 配备了一个内置的开发服务器，旨在提升开发体验。当你执行 rsbuild dev 或 rsbuild preview 命令时，该服务器将启动，并提供页面预览、路由、模块热更新等功能。'
---
...
默认情况下，Rsbuild 服务器的基础路径是 `/`，你可以通过 http://localhost:3000/ 访问 `index.html` 等输出文件，以及 [public 目录](/guide/basic/static-assets#public-folder) 下的资源。
...
| [devServer.setupMiddlewares](https://rspack.rs/zh/config/dev-server#devserversetupmiddlewares)       | [server.setup](/config/server/setup)                             |
...
> 更多配置请参考 [配置总览](/config/index)。

## 中间件 \{#middleware}

Rsbuild 的中间件实现基于 [connect-next](https://github.com/rstackjs/connect-next)，一个轻量级的 HTTP 服务器框架，并使用 Node.js 标准的 `request` 和 `response` 对象来处理 HTTP 交互。

### 注册中间件

Rsbuild 提供了三种方式来注册中间件：

1. 通过 Rsbuild 的 [server.setup](/config/server/setup) 配置项。
