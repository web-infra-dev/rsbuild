...
- 将 `@rsbuild/core` 升级到 2.0 Beta 版本：
...
Rsbuild 依赖的 [http-proxy-middleware](https://github.com/chimurai/http-proxy-middleware) 从 v2 升级到了 v3，个别配置项发生了变化。
...
- 移除 [sockWrite](/api/javascript-api/server-api#sockwrite) 中废弃的 `content-changed` 消息类型，使用 `static-changed` 代替
