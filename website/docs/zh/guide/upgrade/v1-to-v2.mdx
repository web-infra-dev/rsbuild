---
description: '当前文档列出了从 Rsbuild v1 到 2.0 的所有不兼容更新，你可以参考此文档来迁移。'
---
...
[source.decorators.version](/config/source/decorators#decoratorsversion) 的默认值从 `2022-03` 变更为 `2023-11`。
...
当 [output.target](/config/output/target) 为 `node` 时，Rsbuild 2.0 会通过 [output.module](/config/output/module) 默认输出 ESM 产物，并保持 [output.minify](/config/output/minify) 关闭。在 Rsbuild v1 中，默认行为是输出 CommonJS 且开启压缩。
...
你可以根据以下示例进行迁移，或查看 [http-proxy-middleware v3 不兼容更新](https://github.com/chimurai/http-proxy-middleware/blob/master/MIGRATION.md#v3-breaking-changes) 了解更多细节。
...
- 移除 [sockWrite](/api/javascript-api/server-api#sockwrite) 中废弃的 `content-changed` 消息类型。使用 `full-reload` 代替
...
- 如果你通过全局 `logger.override()` 自定义 Rsbuild 日志，升级到 v2 后需要改用 [customLogger](/config/custom-logger)。因为 v2 的实例 logger 不再受全局 `logger.override()` 影响。
