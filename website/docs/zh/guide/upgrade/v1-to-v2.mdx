...
`performance.chunkSplit` 在 Rsbuild 2.0 中已废弃，但暂未移除，仍可继续使用。

推荐使用新的 [splitChunks](/config/split-chunks) 选项代替，新选项对齐了 Rspack 的 [optimization.splitChunks](https://rspack.rs/zh/plugins/webpack/split-chunks-plugin) 配置，并提供了与 `performance.chunkSplit.strategy` 相似的预设选项。
...
- `strategy: 'custom'` + `override` → `splitChunks.preset: 'none'`
