...
如果你发现 tree shaking 没有按照预期生效，可以检查下相关 npm 包的 `sideEffects` 配置是否正确，如果你不了解 `sideEffects` 的作用，或是对 tree shaking 背后的原理感兴趣，可以阅读 [Rspack 官方文档 - Tree Shaking](https://rspack.dev/guide/optimization/tree-shaking)。
