...
当你使用 Rsbuild [JavaScript API](/api/start/index) 时，你可以手动调用 Rsbuild 提供的 [loadEnv](/api/javascript-api/core#loadenv) 方法来读取环境变量，并通过 [source.define](/config/source/define) 配置项注入到代码中。
