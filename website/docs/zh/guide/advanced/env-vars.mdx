...
你可以在 client 代码中使用 `import.meta.env.BASE_URL` 来访问服务端的[基础路径](/guide/basic/server#服务端基础路径)，它由 [server.base](/config/server/base) 配置项决定。
...
在 JS 文件中也可以使用 `import.meta.env.BASE_URL`：
