---
description: 'Rsbuild 默认支持 现代浏览器，同时提供语法降级和 API 降级能力，最低可以兼容到支持 ES5 的旧版浏览器（如 IE11）。'
---

# 浏览器兼容性

Rsbuild 默认支持 [现代浏览器](/guide/advanced/browserslist#default-values)，同时提供语法降级和 API 降级能力，最低可以兼容到支持 ES5 的旧版浏览器（如 IE11）。
