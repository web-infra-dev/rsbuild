---
description: '本章节介绍如何使用 Rsbuild 实现 SSR 功能。'
---
  ...
  environments: {
    ...
    node: {
      ...
      output: {
        ...
        target: 'node',
        // 由 Node.js 从 node_modules 加载运行时依赖
        autoExternal: true,
      },
    },
  },
...
在 SSR 场景中，服务端产物通常运行在 Node.js 中，并与 `node_modules` 一起部署。此时建议优先使用 [output.autoExternal](/config/output/auto-external)，而不是在 `output.externals` 中手动维护依赖列表。开启后，Rsbuild 会将根目录 `package.json` 中声明的依赖标记为 external，并覆盖 `react/jsx-runtime` 这类子路径导入，使运行时依赖、原生模块以及依赖运行时插桩的库可以由 Node.js 直接加载。如果你需要生成完全独立的服务端 bundle，或者部署环境不会安装这些被 external 的依赖，可以保持该选项关闭，或按需配置需要 external 的依赖类型。

## 自定义 Server

Rsbuild 提供了 [Server API](/api/javascript-api/server-api) 和 [Environment API](/guide/advanced/environments#environment-api) 来实现 SSR。
...
如果需要预览 SSR 的线上效果，同样需要修改预览命令。SSR Prod Server 示例参考：[Example](https://github.com/rstackjs/rstack-examples/blob/main/rsbuild/ssr-express/prod-server.mjs)。
