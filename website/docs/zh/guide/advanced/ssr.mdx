...
SSR 的服务端产物通常运行在 Node.js 中。如果部署时会同时安装 `node_modules`，建议将依赖配置为 external，让 Node.js 在运行时直接加载这些依赖，而不是将它们打包进服务端产物。

此时建议使用 [output.autoExternal](/config/output/auto-external)：
...
开启后，Rsbuild 会将根目录 `package.json` 中声明的依赖标记为 external，并覆盖 `react/jsx-runtime` 这类子路径导入，使运行时依赖可以由 Node.js 直接加载。

如果你需要生成完全独立的服务端 bundle，或者部署环境不会安装这些被 external 的依赖，可以保持该选项关闭，或按需配置需要 external 的依赖类型。
