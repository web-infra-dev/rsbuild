---
description: 'Rsbuild 支持同时为多个环境构建产物。你可以使用 environments 来并行构建多个环境，并为每个环境设置不同的 Rsbuild 配置。'
---
...
## 构建指定环境 \{#build-a-specific-environment}
...
# 仅构建 web 环境
rsbuild --environment web

# 构建 web 和 ssr 环境
rsbuild --environment web --environment node

# 构建多个环境可以简写为：
rsbuild --environment web,node
```

## 为指定环境添加插件 \{#plugins-specified-environment}
...
如果你是插件开发者，可查看 [开发 Environment 插件](/plugins/dev/index#environment-plugin) 了解详情。
...
## Environment 上下文 \{#environment-context}
  ...
  setup(api) {
    api.onAfterDevCompile(({ environments }) => {
      for (const name in environments) {
        console.log(name, environments[name]);
      }
    });
  },
});
...
你可以在 [server.setup](/config/server/setup) 或 [自定义 Server](/api/javascript-api/instance#rsbuildcreatedevserver) 中使用 environment API。
...
详细使用情况可参考：[SSR + Express 示例](https://github.com/rstackjs/rstack-examples/tree/main/rsbuild/ssr-express)。
