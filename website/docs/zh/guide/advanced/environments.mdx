...
默认情况下，Rsbuild 会并行构建所有环境。如果你的项目中某一个环境需要依赖其他环境先编译完成，可以将你需要依赖的环境名称添加到 Rspack 的 [dependencies](https://rspack.dev/zh/config/other-options#dependencies) 配置中。

例如，node 环境需要依赖 web 环境先编译完成，可以添加如下配置：
