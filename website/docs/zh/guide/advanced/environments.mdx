...
Rsbuild 支持同时为多种不同的环境构建产物。在 Rsbuild 中，`environment` 可以根据不同的使用场景来设置。例如，不同目标(target)下会有不同的使用场景，如客户端渲染(client)、服务器端渲染(SSR)；
此外，相同的目标下也会有不同的使用场景，如服务器端渲染(SSR)、React Server Components(RSC)。这些场景在以往的构建中，往往需要定义多份配置，通过多次构建完成，现在可以通过 `environment` 配置在一次 Rsbuild 构建中完成多个环境的构建。
