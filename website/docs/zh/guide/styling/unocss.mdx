---
description: 'UnoCSS 是一个灵活可扩展的原子化的 CSS 引擎，所有 CSS 工具类都是通过 preset 提供的。'
---
...
你可以通过 [postcss.config.mjs](https://github.com/webpack/postcss-loader#config) 或 [tools.postcss](/config/tools/postcss) 来注册 `unocss` 的 PostCSS 插件。
...
Rsbuild 默认使用 [Lightning CSS](/guide/styling/css-usage#lightning-css) 添加浏览器前缀，因此你只需要注册 `UnoCSS` 插件。
