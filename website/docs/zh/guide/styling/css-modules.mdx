---
description: 'CSS Modules 让我们能以模块化的方式编写 CSS 代码，并且可以在 JavaScript 文件中导入和使用这些样式。使用 CSS Modules 可以自动生成唯一的类名，隔离不同模块之间的样式，避免类名冲突。'
---
...
:::tip
参考 [自定义类名](#custom-class-names) 来修改生成类名的规则。
:::
...
Rsbuild 支持为 CSS Modules 生成准确的类型声明，你只需要开启 [@rsbuild/plugin-typed-css-modules](https://github.com/rstackjs/rsbuild-plugin-typed-css-modules)，再执行构建命令，Rsbuild 就会为项目中所有的 CSS Modules 文件生成类型声明文件。
