...
test('should dispose old HMR event callbacks after page reload', async ({ page }) => {
  const rsbuild = await createRsbuild({
    cwd: import.meta.dirname,
  });
  ...
  try {
    ...
    server.sockWrite('custom', { event: 'count' });
    await expectPoll(() => page.evaluate('window.__count')).toBe(afterFirst + 1);

    // 2) Modify the source file to trigger an HMR update.
    await fs.writeFile(indexPath, originalSource.replace('console.log(\'hello\')', 'console.log(\'hi\')'));

    // 3) Trigger again. If old callback leaked, we'd observe multiple runs.
    await expectPoll(() => page.evaluate('window.__count')).toBe(afterFirst + 1);
    server.sockWrite('custom', { event: 'count' });
    await expectPoll(() => page.evaluate('window.__count')).toBe(afterFirst + 2);
  } finally {
